# Contract State Machine Documentation

This document provides a comprehensive overview of Hooksmith's contract validation state machine, including JSON schemas and WIT interfaces.

## Overview

Hooksmith implements a schema-driven state machine for contract validation that provides:
...
The following schemas define the structure and validation rules for the contract state machine:
...
The following WIT interfaces expose contract validation functionality:
...
## Integration with WIT & JSON Schema

This implementation demonstrates how JSON Schema and WIT can work together:

1. **JSON Schema Defines the Contract State Machine** - Schemas enforce structure and validation rules
2. **WIT Interface Exposes Contract Validation** - WASM components can validate and transition states
3. **WASM Component Implements Logic** - Components can return schemas, validate states, and apply transitions
4. **Rust Host Uses Both** - Combines schemars and wit-bindgen for type-safe validation

## Benefits

- ✅ **Schema as Single Source of Truth** – JSON Schema defines the valid state machine
- ✅ **Language-agnostic Validation** – Any host that supports WIT/WASM can validate contracts
- ✅ **Deterministic Contract Proofs** – The same logic works inside and outside Git hooks
