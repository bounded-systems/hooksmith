...
use git_hooks::{get_hook_metadata, list_hooks, run_hook_tests};
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Info { hook_name } => match get_hook_metadata(&hook_name) {
            Ok(metadata) => {
        ...
        Commands::Breakdown => {
            ...
            for hook_name in hooks {
                match get_hook_metadata(&hook_name) {
                    Ok(metadata) => match metadata.scope {
            ...
            println!(
                "  Client-side: {} ({:.1}%)",
                client_hooks.len(),
                (client_hooks.len() as f64 / (client_hooks.len() + server_hooks.len()) as f64)
                    * 100.0
            );
            println!(
                "  Server-side: {} ({:.1}%)",
                server_hooks.len(),
                (server_hooks.len() as f64 / (client_hooks.len() + server_hooks.len()) as f64)
                    * 100.0
            );
