...
#[derive(Parser, Debug)]
#[command(name = "github-delete")]
#[command(about = "GitHub Actions stub for delete events (branches/tags)")]
struct Args {
...
fn main() -> Result<()> {
    ...
    println!("::group::🗑️ GitHub Delete Event Handler");
        ...
        if let Ok(payload) = std::fs::read_to_string(&event_path) {
            if let Ok(event_data) = serde_json::from_str::<Value>(&payload) {
                ...
                if let Some(ref_name) = event_data.get("ref") {
                    println!("::info::🗑️ Deleted Reference: {}", ref_name);

                    // Set output for workflow
                    println!("deleted_ref={}", ref_name);
                }
                ...
                if let Some(ref_type) = event_data.get("ref_type") {
                    ...
                    // Set output for workflow
                    println!("deleted_ref_type={}", ref_type);
                }
                ...
                if let Some(sender) = event_data.get("sender") {
                    if let Some(login) = sender.get("login") {
                        println!("::info::👤 Deleted by: {}", login);

                        // Set output for workflow
                        println!("deleted_by={}", login);
                    }
                }
            }
        }
    ...
    // For delete events, this might map to pre-receive and post-receive hooks

    println!("::notice::✅ GitHub delete validation completed");
    Ok(())
}
