...
fn show_usage() {
    ...
    println!(
        "  sync-all-remote-branches --force           # Force recreation of existing worktrees"
    );
    println!();
...
fn worktree_exists(branch_name: &str) -> Result<bool, String> {
    let worktree_path = format!("worktrees/{}", branch_name.replace("/", "\\/"));
...
fn create_worktree(branch_name: &str, force: bool) -> Result<bool, String> {
    ...
    if worktree_exists(branch_name)? {
        if force {
            ...
            log_warning(&format!(
                "Worktree already exists for {}, skipping",
                branch_name
            ));
            return Ok(false);
        }
    ...
    match output {
        Ok(output) => {
            if output.status.success() {
                let action = if local_branch_exists {
                    "existing"
                } else {
                    "new"
                };
...
fn sync_main_branch(dry_run: bool) -> Result<(), String> {
    ...
    if behind_count > 0 {
        log_info(&format!(
            "Main branch is {} commits behind origin/main, updating...",
            behind_count
        ));
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    log_info(&format!(
        "Found {} remote branches to process",
        branches.len()
    ));
    println!();
    ...
    for branch_name in &branches {
        if options.dry_run {
            log_info(&format!(
                "DRY RUN: Would create worktree for branch: {}",
                branch_name
            ));
            created_count += 1;
        } else {
    ...
    if options.dry_run {
        log_info(&format!(
            "DRY RUN: Would create {} worktrees",
            created_count
        ));
    } else {
        ...
        if skipped_count > 0 {
            log_warning(&format!(
                "Skipped {} worktrees (already exist)",
                skipped_count
            ));
        }
        if failed_count > 0 {
