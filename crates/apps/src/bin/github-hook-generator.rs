...
fn get_github_events() -> HashMap<String, String> {
    ...
    events.insert(
        ...
        "Pull request activity (secure context)".to_string(),
    );
    events.insert(
        "workflow_dispatch".to_string(),
        "Manual workflow trigger".to_string(),
    );
    events.insert(
        "workflow_call".to_string(),
        "Workflow called by another workflow".to_string(),
    );
    events.insert(
        "schedule".to_string(),
        "Scheduled workflow runs".to_string(),
    );
    events.insert(
        "repository_dispatch".to_string(),
        "External trigger via API".to_string(),
    );
    events.insert("check_run".to_string(), "Check run activity".to_string());
    events.insert(
        "check_suite".to_string(),
        "Check suite activity".to_string(),
    );
    ...
    events.insert(
        "member".to_string(),
        "Repository member activity".to_string(),
    );
    events.insert(
        "team_add".to_string(),
        "Team added to repository".to_string(),
    );
    events.insert("public".to_string(), "Repository made public".to_string());
    events.insert(
        "organization".to_string(),
        "Organization activity".to_string(),
    );

    // Advanced & Extensible
    events.insert(
        "workflow_run".to_string(),
        "Workflow run completion".to_string(),
    );
    ...
    events.insert(
        ...
        "Project card activity".to_string(),
    );
    events.insert(
        "project_column".to_string(),
        "Project column activity".to_string(),
    );
    ...
    events.insert(
        ...
        "Package registry activity".to_string(),
    );
    events.insert(
        "branch_protection_rule".to_string(),
        "Branch protection rule changes".to_string(),
    );
    events.insert(
        "merge_group".to_string(),
        "Merge queue activity".to_string(),
    );
    events.insert(
    ...
    events.insert(
        ...
        "Pull request review comment".to_string(),
    );
...
fn generate_event_hooks(event_name: &str) -> Result<()> {
    ...
    println!(
        "✅ Generated hooks for {}: pre-{} and post-{}",
