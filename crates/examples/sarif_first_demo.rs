use hooksmith::modules::functional_contract_pipeline::{
    create_audit_policy,
    sarif_roles::{
        AuditAction, AuditPolicy, AuditResult, GitMetadata, QueryCriteria, SarifFirstPipeline,
    },
...
fn demonstrate_individual_roles() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let hook = crate::modules::functional_contract_pipeline::sarif_roles::roles::Hook::new(
        HookEvent::PreCommit,
    );
    ...
    let concern = crate::modules::functional_contract_pipeline::sarif_roles::roles::Concern::new(
        ConcernSymbol::Index,
    );
    ...
    let specifier =
        crate::modules::functional_contract_pipeline::sarif_roles::roles::Specifier::new();
    let contracts = vec![ContractSymbol::new("must-exist")];
    let expectations = specifier.generate_expectations(&contracts);
    println!(
        "   Generated {} expectations from {} contracts",
        expectations.len(),
        contracts.len()
    );
    println!();
    ...
    let sarif_log =
        verifier.verify_and_emit_sarif(&snapshots, &expectations, &HookEvent::PreCommit);
    ...
    let mut stegrapher =
        crate::modules::functional_contract_pipeline::sarif_roles::roles::Stegrapher::new();
    let git_metadata = GitMetadata::new(
        "abc123".to_string(),
        "def456".to_string(),
        HookEvent::PreCommit,
    ...
    println!(
        "   Indexed SARIF with {} entries",
        stegrapher.indexed_entries.len()
    );
    println!();
...
fn demonstrate_audit_policies() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut auditor =
        crate::modules::functional_contract_pipeline::sarif_roles::roles::Auditor::new();
...
fn demonstrate_sarif_querying() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let criteria2 = QueryCriteria::new()
...
fn demonstrate_provenance_tracking() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✅ Indexed {} entries with provenance",
        stegrapher.indexed_entries.len()
    );
    println!(
        "✅ Created {} provenance mappings",
        stegrapher.provenance_map.len()
    );
...
fn create_sample_sarif_log() -> SarifLog {
    use serde_sarif::sarif::{
        ArtifactLocation, Location, Message, PhysicalLocation, Result as SarifResult, Run, Tool,
        ToolComponent,
    };
    use std::collections::HashMap;
    ...
    let results = vec![
        SarifResult::builder()
            ...
            .properties(HashMap::from([
                ...
                (
                    "origin".to_string(),
                    serde_json::Value::String("hook/PreCommit".to_string()),
                ),
                (
                    "severity".to_string(),
        ...
        SarifResult::builder()
            ...
            .properties(HashMap::from([
                (
                    "concern".to_string(),
                    serde_json::Value::String("TreeExecutable".to_string()),
                ),
                (
                    "diff_type".to_string(),
                    serde_json::Value::String("Mismatch".to_string()),
                ),
                (
                    "origin".to_string(),
                    serde_json::Value::String("hook/PreCommit".to_string()),
    ...
    SarifLog::builder().version("2.1.0").runs(vec![run]).build()
}
...
fn demonstrate_slot_based_schema() {
    ...
    for (role, input, output, fails, stateless, sarif) in schema {
        println!(
            "{}\t{}\t{}\t{}\t{}\t{}",
            role, input, output, fails, stateless, sarif
        );
    }
