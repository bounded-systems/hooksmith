...
impl GitOperationsHandler {
    ...
    pub async fn handle_git_push(&mut self, req: GitPushRequest) -> Result<GitOperationEvent> {
        ...
        let branch_name = req.branch.unwrap_or_else(|| {
    ...
    pub async fn handle_git_note_add(
        &mut self,
        req: GitNoteAddRequest,
    ) -> Result<GitOperationEvent> {
        let repo = self.open_repository()?;
        ...
        let message = req
            .message
            .unwrap_or_else(|| "Note added by Hooksmith".to_string());
    ...
    pub async fn handle_git_note_get(
        &mut self,
        req: GitNoteGetRequest,
    ) -> Result<GitOperationEvent> {
        let repo = self.open_repository()?;
        let oid = git2::Oid::from_str(&req.object)?;
        let obj = repo.find_object(oid, None)?;
