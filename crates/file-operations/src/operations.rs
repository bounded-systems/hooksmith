...
impl FileOperationsHandler {
    ...
    pub async fn handle_file_read(&self, req: FileReadRequest) -> Result<FileOperationEvent> {
        ...
        match self.read_file(&path, req.encoding.as_deref()).await {
            ...
            Err(e) => Ok(FileOperationEvent::FileReadResult(FileReadResult {
                request_id: req.request_id,
                success: false,
                content: None,
                size: None,
    ...
    pub async fn handle_file_write(&self, req: FileWriteRequest) -> Result<FileOperationEvent> {
        ...
        match self
            .write_file(
                ...
                req.create_parents.unwrap_or(false),
                req.overwrite.unwrap_or(false),
            )
            .await
        {
            Ok((size, metadata)) => Ok(FileOperationEvent::FileWriteResult(FileWriteResult {
                request_id: req.request_id,
                success: true,
                size: Some(size),
                metadata: Some(metadata),
            ...
            Err(e) => Ok(FileOperationEvent::FileWriteResult(FileWriteResult {
                request_id: req.request_id,
                success: false,
                size: None,
                metadata: None,
    ...
    pub async fn handle_file_delete(&self, req: FileDeleteRequest) -> Result<FileOperationEvent> {
        ...
        match self
            .delete_file(&path, req.recursive.unwrap_or(false))
            .await
        {
            Ok(()) => Ok(FileOperationEvent::FileDeleteResult(FileDeleteResult {
                request_id: req.request_id,
                success: true,
                duration_ms: None,
                error: None,
            })),
            Err(e) => Ok(FileOperationEvent::FileDeleteResult(FileDeleteResult {
                request_id: req.request_id,
                success: false,
                duration_ms: None,
                error: Some(FileOperationError {
    ...
    pub async fn handle_file_exists(&self, req: FileExistsRequest) -> Result<FileOperationEvent> {
        ...
        match self.file_exists(&path).await {
            Ok((exists, metadata)) => Ok(FileOperationEvent::FileExistsResult(FileExistsResult {
                request_id: req.request_id,
                success: true,
                exists,
                metadata: Some(metadata),
            ...
            Err(e) => Ok(FileOperationEvent::FileExistsResult(FileExistsResult {
                request_id: req.request_id,
                success: false,
                exists: false,
                metadata: None,
    ...
    pub async fn handle_file_copy(&self, req: FileCopyRequest) -> Result<FileOperationEvent> {
        ...
        match self
            .copy_file(&source, &destination, req.overwrite.unwrap_or(false))
            .await
        {
            Ok((size, metadata)) => Ok(FileOperationEvent::FileCopyResult(FileCopyResult {
                request_id: req.request_id,
                success: true,
                size: Some(size),
                metadata: Some(metadata),
            ...
            Err(e) => Ok(FileOperationEvent::FileCopyResult(FileCopyResult {
                request_id: req.request_id,
                success: false,
                size: None,
                metadata: None,
    ...
    pub async fn handle_file_move(&self, req: FileMoveRequest) -> Result<FileOperationEvent> {
        ...
        match self
            .move_file(&source, &destination, req.overwrite.unwrap_or(false))
            .await
        {
            Ok(metadata) => Ok(FileOperationEvent::FileMoveResult(FileMoveResult {
                request_id: req.request_id,
                success: true,
                metadata: Some(metadata),
                duration_ms: None,
            ...
            Err(e) => Ok(FileOperationEvent::FileMoveResult(FileMoveResult {
                request_id: req.request_id,
                success: false,
                metadata: None,
                duration_ms: None,
    ...
    ) -> Result<FileOperationEvent> {
        let path = self.resolve_path(&req.path);

        match self
            .create_directory(&path, req.create_parents.unwrap_or(false))
            .await
        {
            Ok(metadata) => Ok(FileOperationEvent::DirectoryCreateResult(
            ...
            Err(e) => Ok(FileOperationEvent::DirectoryCreateResult(
                DirectoryCreateResult {
                    request_id: req.request_id,
                    success: false,
                    metadata: None,
    ...
    pub async fn handle_directory_list(
        ...
        match self
            .list_directory(&path, req.recursive.unwrap_or(false))
            .await
        {
            Ok(files) => Ok(FileOperationEvent::DirectoryListResult(
                DirectoryListResult {
                    request_id: req.request_id,
                    success: true,
                    files: Some(files),
            ...
            Err(e) => Ok(FileOperationEvent::DirectoryListResult(
                DirectoryListResult {
                    request_id: req.request_id,
                    success: false,
                    files: None,
    ...
    pub async fn handle_file_checksum(
        &self,
        req: FileChecksumRequest,
    ) -> Result<FileOperationEvent> {
        let path = self.resolve_path(&req.path);
        let algorithm = req.algorithm.unwrap_or_else(|| "sha256".to_string());

        match self.calculate_checksum(&path, &algorithm).await {
            Ok(checksum) => Ok(FileOperationEvent::FileChecksumResult(FileChecksumResult {
                request_id: req.request_id,
                success: true,
            ...
            Err(e) => Ok(FileOperationEvent::FileChecksumResult(FileChecksumResult {
                request_id: req.request_id,
                success: false,
                checksum: None,
                algorithm: Some(algorithm),
    ...
    async fn read_file(
        ...
        path: &Path,
        encoding: Option<&str>,
    ) -> Result<(String, u64, FileMetadata)> {
        let mut file =
        ...
        let content_str = match encoding {
            ...
            _ => String::from_utf8(content).with_context(|| {
                format!("Failed to decode UTF-8 content from: {}", path.display())
            })?,
        };
    ...
    async fn calculate_checksum(&self, path: &Path, algorithm: &str) -> Result<String> {
        ...
        if metadata.is_dir() {
            return Err(anyhow::anyhow!(
                "Cannot calculate checksum for directory: {}",
                path.display()
            ));
        }
        ...
        let mut hasher = match algorithm.to_lowercase().as_str() {
            ...
            "md5" => {
                return Err(anyhow::anyhow!(
                    "MD5 not supported - use SHA256 or SHA1 instead"
                ));
            }
            _ => {
                return Err(anyhow::anyhow!(
