...
impl ScopeAwareContractValidator {
    ...
    fn detect_scopes(&self, current_commit_sha: &str) -> Result<Vec<ValidationScope>> {
        ...
        let scopes_needing_validation = self.scope_manager.get_scopes_needing_validation(current_commit_sha)?;
        ...
        for scope_ref in scope_refs {
            ...
            for contract_id in &scope_ref.contract_ids {
                ...
                let scope = ValidationScope {
                    ...
                    scope_type: if scope_ref.name == "project-root" { "root".to_string() } else { format!("subtree:{}", scope_ref.name) },
                    contract_ids: vec![contract_id.clone()],
                    cache_key,
                    path: if scope_ref.name == "project-root" { "".to_string() } else { scope_ref.name.clone() },
    ...
    fn validate_scope(&self, scope: &ValidationScope, current_commit_sha: &str) -> Result<ValidationResult> {
    ...
    fn run_pipeline(&self, current_commit_sha: &str) -> Result<Vec<ValidationResult>> {
        ...
        for scope in &scopes {
            let status = if scope.needs_validation { "🔄 needs validation" } else { "✅ up to date" };
            println!("  - {} (contracts: {:?}) - {}", 
    ...
    fn initialize_scopes(&self, base_commit_sha: &str) -> Result<()> {
        ...
        self.ref_manager.initialize_project_scopes(base_commit_sha)?;
    ...
    fn export_scope_status(&self) -> Result<Value> {
        ...
        for scope_ref in &scope_refs {
            let status = if scope_ref.last_validated.is_some() { "✅ validated" } else { "⏳ pending" };
            println!("  - {}: {} ({})", scope_ref.name, status, scope_ref.commit_sha);
        }
        ...
        "validate" => {
            ...
            if failed_results.is_empty() {
                ...
                for result in failed_results {
                    println!("  - {}: {} violations", result.scope.name, result.violations.len());
                }
