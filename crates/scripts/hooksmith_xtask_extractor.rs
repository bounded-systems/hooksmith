...
fn run_hooksmith_xtask_extraction(
    args: &[String],
) -> Result<XtaskExtractor, Box<dyn std::error::Error>> {
...
fn analyze_extraction(
    plan: &XtaskExtractionPlan,
) -> Result<ExtractionAnalysis, Box<dyn std::error::Error>> {
...
fn analyze_contract_impact(
    source_path: &str,
) -> Result<ContractImpact, Box<dyn std::error::Error>> {
...
fn export_sha_mapping(
    ...
    for line in commits.lines() {
        if let Some(short_sha) = line.split_whitespace().next() {
            let full_sha_output = Command::new("git").args(&["rev-parse", short_sha]).output();
...
fn generate_xtask_report(extractor: &XtaskExtractor) {
    ...
    println!(
        "  • Safety Score: {:.1}%",
        extractor.analysis_result.safety_score * 100.0
    );
    println!(
        "  • Affected Commits: {}",
        extractor.analysis_result.affected_commits
    );
    println!(
        "  • Affected Files: {}",
        extractor.analysis_result.affected_files
    );
    println!(
        "  • Estimated Savings: {}",
        extractor.analysis_result.estimated_savings
    );
    ...
    println!(
        "  • Invalidation Risk: {:.1}%",
        extractor.analysis_result.contract_impact.invalidation_risk * 100.0
    );
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 3 {
        println!(
            "Usage: {} --source <path> --target <repo> [options]",
            args[0]
        );
        println!();
        println!("Options:");
        ...
        println!("Example:");
        println!(
            "  {} --source crates/tooling/dircheck --target hooksmith-dircheck --mermaid",
            args[0]
        );
        return Ok(());
    }
