...
fn main() -> Result<()> {
    ...
    println!(
        "🎉 All {} agreement(s) validated successfully!",
        agreements.len()
    );
    Ok(())
}
...
fn validate_one(repo: &Repository, path: &Path, cli_ref: &str, ci: bool) -> Result<()> {
    ...
    require(
        &ag.precedence == "allow-overrides-reject",
        "precedence must be allow-overrides-reject",
    )?;
    require(
        &ag.default_action == "reject",
    ...
    let names = match scope {
        ...
        "subtree" => {
            let p = ag
                ...
                .path
                .as_deref()
                .ok_or_else(|| anyhow!("subject.path required for subtree"))?;
...
fn decision_check(
    ...
    for (name, kind) in names {
        let allowed = match (scope, kind) {
            ...
            ("top-level", ObjectType::Blob) => allow_files
                .iter()
                .any(|p| p == name || (star_re.is_match(p) && star_match_root(p, name))),
            ...
            (_, ObjectType::Blob) => {
                ...
                if allow_files.iter().any(|p| p == name) {
                    ...
                    allow_files.iter().any(|p| {
                        if star_re.is_match(p) && !name.contains('/') {
                            star_match_root(p, name)
                        } else if star_re.is_match(p) {
                            star_match_basename(p, name)
                        } else {
                            false
                        }
                    })
...
fn compute_digest(
    ...
    h1.update(subject.as_bytes());
    let a = h1.finalize();
    let mut h2 = Sha256::new();
    h2.update(rules_str.as_bytes());
    let b = h2.finalize();
