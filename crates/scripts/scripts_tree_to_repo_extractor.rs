...
fn analyze_tree_to_repo_extraction(source_path: &str, target_repo: &str, dry_run: bool) -> Result<TreeToRepoExtractor, Box<dyn std::error::Error>> {
    println!("🌳 Analyzing tree-to-repo extraction for '{}' -> '{}'...", source_path, target_repo);
    if dry_run {
    ...
    let extraction_plan = create_extraction_plan(source_path, target_repo, &current_state, dry_run)?;
...
fn analyze_repository_state(source_path: &str) -> Result<RepositoryState, Box<dyn std::error::Error>> {
    // Get current branch
    ...
    let affected_commits_output = Command::new("git")
        ...
        .output()?;
    let affected_commits = String::from_utf8(affected_commits_output.stdout)?.lines().count() as u32;
    
    // Count affected files
    let affected_files_output = Command::new("git")
        .args(&["log", "--name-only", "--pretty=format:", "--", source_path])
        .output()?;
    let affected_files = String::from_utf8(affected_files_output.stdout)?.lines().count() as u32;
...
fn find_contract_dependencies(source_path: &str) -> Result<Vec<String>, Box<dyn std::error::Error>> {
...
fn create_extraction_plan(source_path: &str, target_repo: &str, state: &RepositoryState, dry_run: bool) -> Result<ExtractionPlan, Box<dyn std::error::Error>> {
    ...
    let dry_run_preview = create_dry_run_preview(source_path, state, &affected_files, &sha_mapping)?;
    
    // Generate mermaid diagram
    let mermaid_diagram = generate_mermaid_diagram(source_path, target_repo, &extraction_type, &affected_commits, &affected_files);
...
fn create_dry_run_preview(source_path: &str, state: &RepositoryState, affected_files: &[String], sha_mapping: &HashMap<String, String>) -> Result<DryRunPreview, Box<dyn std::error::Error>> {
    ...
    let estimated_savings = calculate_estimated_savings(affected_files.len(), state.affected_commits);
...
fn generate_mermaid_diagram(source_path: &str, target_repo: &str, extraction_type: &ExtractionType, affected_commits: &[String], affected_files: &[String]) -> String {
...
fn determine_extraction_type(source_path: &str, state: &RepositoryState) -> Result<ExtractionType, Box<dyn std::error::Error>> {
...
fn create_sha_mapping(affected_commits: &[String]) -> Result<HashMap<String, String>, Box<dyn std::error::Error>> {
...
fn create_safety_checks(source_path: &str, state: &RepositoryState, extraction_type: &ExtractionType) -> Vec<SafetyCheck> {
    ...
    checks.push(SafetyCheck {
        ...
        status: if path_exists { CheckStatus::Passed } else { CheckStatus::Critical },
...
fn generate_execution_steps(source_path: &str, target_repo: &str, extraction_type: &ExtractionType) -> Vec<String> {
    ...
    match extraction_type {
        ExtractionType::FilterRepo => {
            ...
            steps.push(format!("   git filter-repo --subdirectory-filter {}", source_path));
            steps.push("4. Add the new remote".to_string());
            steps.push(format!("   git remote add origin https://github.com/yourorg/{}", target_repo));
            ...
            steps.push(format!("   git filter-repo --path {} --invert-paths", source_path));
        }
        ExtractionType::SubtreeSplit => {
            steps.push("1. Create a subtree split branch".to_string());
            steps.push(format!("   git subtree split --prefix={} -b {}-history", source_path, target_repo));
...
fn analyze_contract_implications(state: &RepositoryState, plan: &ExtractionPlan) -> Result<ContractAnalysis, Box<dyn std::error::Error>> {
...
fn perform_safety_analysis(state: &RepositoryState, plan: &ExtractionPlan) -> Result<SafetyAnalysis, Box<dyn std::error::Error>> {
...
fn generate_execution_plan(plan: &ExtractionPlan, safety: &SafetyAnalysis, dry_run: bool) -> Vec<String> {
...
fn generate_tree_to_repo_extraction_report(extractor: &TreeToRepoExtractor) {
    ...
    println!("  • Current branch: {}", extractor.current_state.current_branch);
    println!("  • Total commits: {}", extractor.current_state.total_commits);
    println!("  • Affected commits: {}", extractor.current_state.affected_commits);
    println!("  • Affected files: {}", extractor.current_state.affected_files);
    println!("  • Contract dependencies: {}", extractor.current_state.contract_dependencies.len());
    ...
    println!("  • Extraction type: {:?}", extractor.extraction_plan.extraction_type);
    println!("  • Affected commits: {}", extractor.extraction_plan.affected_commits.len());
    println!("  • Affected files: {}", extractor.extraction_plan.affected_files.len());
    println!("  • SHA mappings: {}", extractor.extraction_plan.sha_mapping.len());
    
    // Show dry-run preview
    if extractor.dry_run_mode {
        println!("\n🔍 Dry-Run Preview:");
        println!("  📊 Before Extraction:");
        println!("    • Total commits: {}", extractor.extraction_plan.dry_run_preview.before_state.total_commits);
        println!("    • Total files: {}", extractor.extraction_plan.dry_run_preview.before_state.total_files);
        println!("    • Repository size: {}", extractor.extraction_plan.dry_run_preview.before_state.total_size);
        println!("    • Crate count: {}", extractor.extraction_plan.dry_run_preview.before_state.crate_count);
        
        println!("  📊 After Extraction:");
        println!("    • Total commits: {}", extractor.extraction_plan.dry_run_preview.after_state.total_commits);
        println!("    • Total files: {}", extractor.extraction_plan.dry_run_preview.after_state.total_files);
        println!("    • Repository size: {}", extractor.extraction_plan.dry_run_preview.after_state.total_size);
        println!("    • Crate count: {}", extractor.extraction_plan.dry_run_preview.after_state.crate_count);
        
        println!("  💾 Estimated Savings: {}", extractor.extraction_plan.dry_run_preview.estimated_savings);
        println!("  🔗 SHA Mappings: {}", extractor.extraction_plan.dry_run_preview.sha_mapping.len());
    }
    
    // Show safety analysis
    println!("\n🔒 Safety Analysis:");
    println!("  • Overall safety score: {:.1}%", extractor.safety_analysis.overall_safety_score * 100.0);
    
    let passed_checks = extractor.safety_analysis.checks.iter().filter(|c| matches!(c.status, CheckStatus::Passed)).count();
    let warning_checks = extractor.safety_analysis.checks.iter().filter(|c| matches!(c.status, CheckStatus::Warning)).count();
    let failed_checks = extractor.safety_analysis.checks.iter().filter(|c| matches!(c.status, CheckStatus::Failed)).count();
    let critical_checks = extractor.safety_analysis.checks.iter().filter(|c| matches!(c.status, CheckStatus::Critical)).count();
    ...
    println!("\n📋 Contract Analysis:");
    println!("  • Affected contracts: {}", extractor.contract_analysis.affected_contracts.len());
    println!("  • Contract hashes: {}", extractor.contract_analysis.contract_hashes.len());
    println!("  • Invalidation risk: {:.1}%", extractor.contract_analysis.invalidation_risk * 100.0);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 3 {
        ...
        println!("Example: {} crates/tooling/dircheck hooksmith-dircheck --dry-run", args[0]);
        return Ok(());
    }
    ...
    println!("=========================");
    println!("Analyzing extraction from '{}' to '{}'...", source_path, target_repo);
    if dry_run {
        println!("🔍 DRY-RUN MODE: No actual changes will be made");
    }
    println!();
