    ...
    for (source, destination) in &moves {
        ...
        if Path::new(&source_path).exists() {
            ...
            if let Some(parent) = Path::new(&dest_path_full).parent() {
                if !parent.exists() {
                    fs::create_dir_all(parent).context(format!("Failed to create parent directory for: {}", dest_path_full))?;
                }
            }
            
            // Move the file/directory
            fs::rename(&source_path, &dest_path_full).context(format!("Failed to move {} to {}", source_path, dest_path_full))?;
