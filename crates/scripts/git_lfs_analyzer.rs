            ...
            if let Ok(size_str) = String::from_utf8(size_output.stdout) {
                if let Ok(size) = u64::from_str(size_str.trim()) {
                    ...
                    if should_track {
                        ...
                        let lfs_recommendation =
                            generate_lfs_recommendation(&path, size, &file_type);
...
fn analyze_lfs_candidate(path: &str, size: u64) -> (bool, String) {
    ...
    if path_lower.contains(".pdf") && size > 5 * 1024 * 1024 {
        // 5 MB PDFs
        return (true, "Large PDF file".to_string());
    }

    if path_lower.contains(".png") && size > 2 * 1024 * 1024 {
        // 2 MB PNGs
        return (true, "Large image file".to_string());
    }

    if path_lower.contains(".jpg") && size > 5 * 1024 * 1024 {
        // 5 MB JPGs
        return (true, "Large image file".to_string());
    }
...
fn generate_lfs_analysis_report(
    candidates: &[LfsCandidate],
    binary_hooks: &[BinaryHookInfo],
) -> LfsAnalysisResult {
    ...
    if !binary_hooks.is_empty() {
        println!(
            "\n🔧 Binary Hook Reuse Opportunities ({}):",
            binary_hooks.len()
        );
    ...
    println!(
        "  • Potential LFS savings: {:.2} MB",
        total_lfs_savings as f64 / (1024.0 * 1024.0)
    );
