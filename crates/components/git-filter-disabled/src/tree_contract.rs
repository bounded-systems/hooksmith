...
impl TreeEntryContract {
    ...
    pub fn new_with_attributes(
        ...
        if let Some(ref attrs) = attributes {
            ...
            if Self::is_generated_file(&filename) {
                errors.push(format!(
                    "Generated file '{filename}' must have 'linguist-generated=true' attribute"
                ));
                valid = false;
            }
        }
    ...
    fn validate_attributes(filename: &str, attributes: &[String]) -> (bool, Vec<String>) {
        ...
        if !Self::is_generated_file(filename) && has_linguist_generated {
            errors.push(format!(
                "Non-generated file '{filename}' should not have 'linguist-generated=true' attribute"
            ));
        }
...
impl TreeObjectContract {
    ...
    pub fn summary(&self) -> String {
        ...
        if valid_entries == total_entries {
            format!("✅ Tree valid ({total_entries} entries)")
        } else {
            format!("❌ Tree invalid ({valid_entries} valid / {total_entries} total entries)")
        }
    }
