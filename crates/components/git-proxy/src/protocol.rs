...
use git2::{Cred, Direction, FetchOptions, PushOptions, Remote, Repository};
...
use crate::{
    ...
    GitPullRequest, GitPullResult, GitPushRequest, GitPushResult,
};
...
impl HttpSmartProtocol {
    ...
    async fn clone_repository(&self) -> Result<Repository> {
        ...
        Repository::clone(&self.config.upstream_url, repo_path).with_context(|| {
            format!(
    ...
    async fn forward_push_to_upstream(&self, request: &GitPushRequest) -> Result<bool> {
        ...
        info!(
            "Successfully pushed {} refs to upstream",
            request.refs.len()
        );
        Ok(true)
    }
    ...
    async fn forward_pull_from_upstream(&self, request: &GitPullRequest) -> Result<bool> {
        ...
        info!(
            "Successfully pulled {} refs from upstream",
            request.refs.len()
        );
        Ok(true)
    }
    ...
    async fn forward_fetch_from_upstream(&self, request: &GitFetchRequest) -> Result<bool> {
        ...
        info!(
            "Successfully fetched {} refs from upstream",
            request.refs.len()
        );
        Ok(true)
    }
...
impl GitProtocolHandler for HttpSmartProtocol {
    ...
    fn protocol_info(&self) -> ProtocolInfo {
        ProtocolInfo {
            ...
            auth_methods: vec!["basic".to_string(), "token".to_string()],
        }
    }
}
...
impl SshProtocol {
    ...
    async fn forward_push_to_upstream(&self, request: &GitPushRequest) -> Result<bool> {
        ...
        info!(
            "Successfully pushed {} refs to upstream via SSH",
            request.refs.len()
        );
        Ok(true)
    }
    ...
    async fn forward_pull_from_upstream(&self, request: &GitPullRequest) -> Result<bool> {
        ...
        info!(
            "Successfully pulled {} refs from upstream via SSH",
            request.refs.len()
        );
        Ok(true)
    }
    ...
    async fn forward_fetch_from_upstream(&self, request: &GitFetchRequest) -> Result<bool> {
        ...
        info!(
            "Successfully fetched {} refs from upstream via SSH",
            request.refs.len()
        );
        Ok(true)
    }
...
impl GitProtocolHandler for SshProtocol {
    ...
    fn protocol_info(&self) -> ProtocolInfo {
        ProtocolInfo {
            ...
            auth_methods: vec!["ssh-key".to_string(), "ssh-agent".to_string()],
        }
    }
}
...
mod tests {
    ...
    fn test_protocol_detection() {
        assert_eq!(
            ProtocolFactory::detect_protocol("ssh://git@github.com/user/repo.git"),
            GitProtocol::Ssh
        );
        assert_eq!(
            ProtocolFactory::detect_protocol("git@github.com:user/repo.git"),
            GitProtocol::Ssh
        );
        assert_eq!(
            ProtocolFactory::detect_protocol("https://github.com/user/repo.git"),
            GitProtocol::Https
        );
        assert_eq!(
            ProtocolFactory::detect_protocol("http://github.com/user/repo.git"),
            GitProtocol::Http
        );
    }
