        ...
        Commands::Tools { tool_command } => {
            ...
            match tool_command {
                ...
                ToolCommands::Create { branch } => {
                    ...
                    if result.success {
                        println!("✅ Worktree created successfully using {}", result.tool_used);
                ...
                ToolCommands::Devspace { devspace_command } => {
                    match devspace_command {
        ...
        Commands::Kube { kube_command } => {
            match kube_command {
                ...
                KubeCommands::Create { branch, namespace } => {
                    println!("Creating WorktreeChangeRequest for branch: {} in namespace: {}", branch, namespace);
                    
                    let crd = worktree_runner::kube_crd::WorktreeChangeRequest::create(&branch);
                    let _yaml = serde_yaml::to_string(&crd)
