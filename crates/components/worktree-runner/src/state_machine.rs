...
<<<<<<< HEAD
        ...
        for branch in all_branches {
            ...
            self.populate_remote_domain(&mut crd, &remote_branches)
                .await;
...
<<<<<<< HEAD
        ...
        for crd in crds.iter_mut() {
            ...
            if let Some(action) = action {
                info!(
                    "Executing action {:?} for branch {}",
                    action, crd.spec.branch
                );
...
<<<<<<< HEAD
    async fn execute_action(
        ...
        match action {
            WorktreeAction::CreateBranch => self.create_branch(&crd.spec.branch).await,
            WorktreeAction::CreateWorktree => self.create_worktree(&crd.spec.branch).await,
            WorktreeAction::PushBranch => self.push_branch(&crd.spec.branch).await,
            WorktreeAction::CreatePr => self.create_pull_request(&crd.spec.branch).await,
            WorktreeAction::MergePr => self.merge_pull_request(crd).await,
            WorktreeAction::ResolveConflicts => self.resolve_conflicts(&crd.spec.branch).await,
            WorktreeAction::RebaseMain => self.rebase_main(&crd.spec.branch).await,
            WorktreeAction::CleanupWorktree => self.cleanup_worktree(&crd.spec.branch).await,
            WorktreeAction::RemoveBranch => self.remove_branch(&crd.spec.branch).await,
            WorktreeAction::ResetMain => self.reset_main().await,
            // New enhanced actions
            WorktreeAction::ExtractFeature => self.extract_feature(&crd.spec.branch).await,
            WorktreeAction::RecoverDirtyMain => self.recover_dirty_main().await,
            WorktreeAction::SquashCommits => self.squash_commits(&crd.spec.branch).await,
            WorktreeAction::UpdateBranch => self.update_branch(&crd.spec.branch).await,
            WorktreeAction::SyncRemote => self.sync_remote(&crd.spec.branch).await,
            WorktreeAction::MarkStale => self.mark_stale(&crd.spec.branch).await,
            WorktreeAction::ForcePush => self.force_push(&crd.spec.branch).await,
            WorktreeAction::AbortRebase => self.abort_rebase(&crd.spec.branch).await,
            WorktreeAction::StashChanges => self.stash_changes(&crd.spec.branch).await,
            WorktreeAction::ApplyStash => self.apply_stash(&crd.spec.branch).await,
            WorktreeAction::CreateBackup => self.create_backup(&crd.spec.branch).await,
            WorktreeAction::RestoreBackup => self.restore_backup(&crd.spec.branch).await,
...
<<<<<<< HEAD
    async fn populate_pr_domain(
        &mut self,
        crd: &mut WorktreeChangeRequest,
        prs: &HashMap<String, (i32, PrState)>,
    ) {
        let branch = &crd.spec.branch;
...
<<<<<<< HEAD
        ...
        if domains.local.exists
            && domains.local.ahead > 0
            && domains.local.behind == 0
            && !domains.pr.exists
        {
