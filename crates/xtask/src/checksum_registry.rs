...
impl ChecksumRegistry {
    ...
    pub fn load() -> Result<Self> {
        ...
        let content = fs::read_to_string(registry_path)
            .context(format!("Failed to read registry: {}", registry_path.display()))?;
        
        // Strip comments and parse JSON
        let json_content = strip_jsonc_comments(&content);
        let registry: ChecksumRegistry = serde_json::from_str(&json_content)
            .context("Failed to parse checksum registry")?;
    ...
    pub fn save(&self) -> Result<()> {
        ...
        let json_content = serde_json::to_string_pretty(self)
            .context("Failed to serialize checksum registry")?;
        ...
        fs::write(registry_path, jsonc_content)
            .context(format!("Failed to write registry: {}", registry_path.display()))?;
...
impl UpdateReport {
    ...
    pub fn add_updated(&mut self, path: &str, old_checksum: &str, new_checksum: &str) {
        self.updated.push((path.to_string(), old_checksum.to_string(), new_checksum.to_string()));
    }
        ...
        if !in_string {
            ...
            if c == '/' && i + 1 < content.len() && content.chars().nth(i + 1) == Some('*') {
                ...
                while i + 1 < content.len() {
                    if content.chars().nth(i) == Some('*') && content.chars().nth(i + 1) == Some('/') {
                        i += 2;
                        break;
                    }
                    i += 1;
