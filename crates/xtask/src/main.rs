...
enum Commands {
    ...
    /// Generate .gitignore from repository state
    GenGitignore {
        /// Output file path
        #[arg(long, default_value = ".gitignore")]
        output: String,
        /// Whether to validate against existing .gitignore
        #[arg(long)]
        validate: bool,
        ...
        #[arg(long)]
        minimal: bool,
        /// Write generated content to file (atomic rename, local only)
        #[arg(long)]
        write: bool,
    },
...
fn generate_gitignore(
    ...
    // Check if we're in CI (never allow --write in CI)
    let in_ci = std::env::var("CI").is_ok();
    if write && in_ci {
        return Err(anyhow::anyhow!(
            "--write flag not allowed in CI environment"
        ));
    }
    ...
    let content = if minimal {
        ...
        generate_full_gitignore()?
    };

    if validate {
        validate_gitignore_content(output, &content)?;
    } else if dry_run {
        println!("Would generate .gitignore with content:");
        println!("{}", content);
    } else if write {
        write_gitignore_atomic(output, &content)?;
    } else {
        // Default behavior: just show what would be generated
        println!("Would generate .gitignore with content:");
        println!("{}", content);
    }
...
// ---------- Gitignore Generation Functions

fn generate_full_gitignore() -> Result<String> {
    let content = r#"# @generated by xtask gen-config
# @checksum: e902a214
...
############################
# Test artifacts & demos
############################
/test-enhanced-gen-files/
/scripts/test-*.sh

############################
# Build artifacts (docs etc.)
############################
*.pdf
*.epub
*.html
...
"#;
    ...
    canonicalize_gitignore_content(content)
}

fn generate_canonical_gitignore() -> Result<String> {
    use std::process::Command;

    // Get the actual Git tree entries from HEAD
    let output = Command::new("git")
    ...
    if !output.status.success() {
        return Err(anyhow::anyhow!(
            "Failed to get Git tree entries: {}",
    ...
    let ignorable_patterns = vec![
        ...
        // EditorConfig backups
        (".editorconfig.bak", "/.editorconfig.bak"),
        // Lefthook runtime
        (".lefthook", "/.lefthook/"),
        ...
        (".envrc", ".envrc"),
        // Test artifacts & demos
        ("test-enhanced-gen-files", "/test-enhanced-gen-files/"),
        ...
        ("generated_file_demo", "/generated_file_demo"),
        // Timestamped/temporary blobs
        ("2025", "2025*"),
        // Git worktree helpers
        ("worktrees", "/worktrees/"),
        ...
        ("git_worktree_wrapper", "git_worktree_wrapper"),
        ("update_worktrees", "update_worktrees*"),
        ("safe_worktree_cleanup_rs", "safe_worktree_cleanup_rs"),
        ("verify_worktree_1to1_rs", "verify_worktree_1to1_rs"),
        ("validate-file-extensions", "validate-file-extensions"),
        ("*.rs.bin", "*.rs.bin"),
    ];

    let mut gitignore_lines = vec![
        "# @generated by xtask gen-config --minimal",
        "# @checksum: e902a214",
        "# Based on actual Git tree state",
        "",
    ];
    ...
    for (_entry_name, pattern) in ignorable_patterns {
        ...
        let should_include = entries.iter().any(|entry| {
            if pattern.ends_with('/') {
                ...
                *entry == &pattern[..pattern.len() - 1]
            } else if pattern.starts_with('/') {
                // Root-anchored pattern - exact match
                *entry == &pattern[1..]
            } else if pattern.contains('*') {
                // Simple glob pattern - check if entry starts with the pattern before *
                let pattern_prefix = pattern.split('*').next().unwrap_or("");
                entry.starts_with(pattern_prefix)
            } else {
                // Exact match
                *entry == pattern
            }
        ...
        if should_include {
            gitignore_lines.push(pattern);
        }
    }

    // Sort the patterns for consistent output
    gitignore_lines.sort();

    canonicalize_gitignore_content(&gitignore_lines.join("\n"))
}

fn canonicalize_gitignore_content(content: &str) -> Result<String> {
    ...
    let mut rule_lines: Vec<&str> = lines.iter().skip(header_lines.len()).cloned().collect();

    rule_lines.sort();

    // Reject empty file unless there's at least one rule
    if rule_lines.is_empty() {
        return Err(anyhow::anyhow!(
            "No gitignore rules generated - empty file not allowed"
        ));
    }

    // Combine header and sorted rules
    let mut result = header_lines.join("\n");
    if !result.is_empty() {
        result.push('\n');
    }
    result.push_str(&rule_lines.join("\n"));
    ...
    // Convert to Unix newlines only
    result = result.replace("\r\n", "\n");

    Ok(result)
}

fn validate_gitignore_content(output: &str, expected_content: &str) -> Result<()> {
    let existing_content = match std::fs::read_to_string(output) {
        Ok(content) => content,
        Err(_) => {
            println!("⚠️  No existing .gitignore found for validation");
            return Ok(());
        }
    };

    // Normalize both contents for comparison
    let normalized_existing = canonicalize_gitignore_content(&existing_content)?;
    let normalized_expected = canonicalize_gitignore_content(expected_content)?;

    if normalized_existing == normalized_expected {
        println!("✅ .gitignore validation passed");
        Ok(())
    } else {
        // Generate diff for better error reporting
        let diff = generate_diff(&normalized_existing, &normalized_expected);
        println!("❌ .gitignore validation failed");
        println!("Diff (first 40 lines):");
        println!("{}", diff.lines().take(40).collect::<Vec<_>>().join("\n"));
        println!("Run 'cargo xtask gen-gitignore --write' to fix");
        Err(anyhow::anyhow!("Gitignore validation failed"))
    }
}

fn write_gitignore_atomic(output: &str, content: &str) -> Result<()> {
    ...
    println!("✅ Generated .gitignore at {}", output);
    Ok(())
}

fn generate_diff(existing: &str, expected: &str) -> String {
    let existing_lines: Vec<&str> = existing.lines().collect();
    let expected_lines: Vec<&str> = expected.lines().collect();

    let mut diff = String::new();
    let max_lines = std::cmp::max(existing_lines.len(), expected_lines.len());
    ...
    if let Some(specific_dir) = dir {
        ...
        match generate_gitignore_for_directory(&dir_path, local_only, max_patterns, dry_run) {
            ...
            Ok(false) => {
                skipped_count += 1;
                println!(
                    "⏭️  Skipped .gitignore for {} (no untracked files)",
                    specific_dir
                );
            }
            Err(e) => {
                println!(
                    "❌ Failed to generate .gitignore for {}: {}",
                    specific_dir, e
                );
                if !dry_run {
                    return Err(e);
        ...
        for line in output_str.lines() {
            if !line.is_empty() {
                ...
                if file_path.parent().is_none()
                    || file_path.parent().unwrap() == Path::new(".")
...
fn validate_tree_gitignore_files(root_path: &Path, all: bool, dir: Option<&str>) -> Result<()> {
...
fn validate_single_gitignore_file(gitignore_path: &Path) -> Result<()> {
    ...
    for (line_num, line) in lines.iter().enumerate() {
        ...
        if line.contains("**") {
            return Err(anyhow::anyhow!(
                "Line {}: Recursive pattern '{}' not allowed in tree-scoped .gitignore",
                line_num,
                line
            ));
        }
        ...
        if !line.starts_with('/') {
            return Err(anyhow::anyhow!(
                "Line {}: Pattern '{}' must start with '/' for local scope",
                line_num,
                line
            ));
        }
        ...
        if *line == "/*" || *line == "/.*" {
            return Err(anyhow::anyhow!(
                "Line {}: Too generic pattern '{}' not allowed",
                line_num,
                line
            ));
        }
