...
fn generate_gitignore(output: &str, validate: bool, dry_run: bool, minimal: bool, write: bool) -> Result<()> {
    // Check if we're in CI (never allow --write in CI)
    let in_ci = std::env::var("CI").is_ok();
    if write && in_ci {
        return Err(anyhow::anyhow!("--write flag not allowed in CI environment"));
    }
...
fn generate_canonical_gitignore() -> Result<String> {
    ...
    if !output.status.success() {
        return Err(anyhow::anyhow!("Failed to get Git tree entries: {}", 
...
fn canonicalize_gitignore_content(content: &str) -> Result<String> {
    ...
    let mut rule_lines: Vec<&str> = lines.iter()
    ...
    if rule_lines.is_empty() {
        return Err(anyhow::anyhow!("No gitignore rules generated - empty file not allowed"));
    }
