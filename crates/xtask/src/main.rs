...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::GenGitignore { output, validate, dry_run, minimal } => {
            generate_gitignore(&output, validate, dry_run, minimal)?;
        }
...
fn generate_gitignore(output: &str, validate: bool, dry_run: bool, minimal: bool) -> Result<()> {
    let gitignore_content = r#"# @generated by xtask gen-config
# @checksum: e902a214
...
"#;
    ...
    if validate {
        // Read existing .gitignore and compare
        if let Ok(existing_content) = std::fs::read_to_string(output) {
            if existing_content.trim() == gitignore_content.trim() {
                println!("✅ .gitignore is up to date");
                return Ok(());
            } else {
                println!("❌ .gitignore differs from expected content");
                println!("Run 'cargo xtask gen-gitignore' to update");
    ...
    std::fs::write(output, gitignore_content)?;
...
fn generate_minimal_gitignore() -> Result<String> {
    ...
    for (entry_name, pattern) in ignorable_patterns {
        if entries.iter().any(|e| e.contains(entry_name)) {
