...
fn validate_single_hook(hook_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let concerns = json["concerns"]
        .as_array()
        .ok_or("Missing 'concerns' field")?;
    let valid_concerns = [
        "blob",
        "tree",
        "commit",
    ...
    let mut concern_strings: Vec<&str> = concerns.iter().filter_map(|c| c.as_str()).collect();
    ...
    if !binary_path.exists() {
        println!(
            "⚠️  Binary '{}' not found in target/release/ - skipping validation during build",
            bin
        );
        return Ok(());
    }
