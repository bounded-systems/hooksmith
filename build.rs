...
fn validate_single_hook(hook_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let valid_concerns = [
        ...
        "head-pointer", "index-entry", "index-file", "index-stage", "ref-branch", "ref-remote-branch", "ref-tag", "ref-packed", "note-ref", "attr-repo-only", "ignore-repo-only", "config-local", "hook-script", "hook-lifecycle", "ref-log", "ref-log-entry", "worktree-meta", "worktree-lock", "rebase-plan", "rr-cache-entry", "merge-state", "merge-head", "orig-head-pointer", "commit-message-draft", "fetch-head-pointer", "repo-description", "fs-monitor-state", "shallow-clone-depth", "tree-ignore-pattern", "ignore-global-pattern", "attr-pattern", "attr-global", "remote-origin", "remote-config", "push-strategy-config", "credential-helper-config", "remote-url-alias", "stash-entry", "stash-ref", "stash-meta", "rebase-step", "merge-conflict-marker", "bisect-state", "bisect-log", "tag-object", "worktree-index", "worktree-branch-link", "hook-trigger", "index-conflict", "index-mode", "config-global", "config-system",
