            ...
            if path.ends_with(".rs") {
                ...
                if let Ok(size_str) = String::from_utf8(size_output.stdout) {
                    if let Ok(blob_size) = u64::from_str(size_str.trim()) {
                        ...
                        let optimization_recommendation = generate_optimization_recommendation(
                            &path,
                            blob_size,
                            complexity_score,
                        );
...
fn get_line_count(path: &str) -> Option<u32> {
    ...
    let output = Command::new("wc").args(&["-l", path]).output();
...
fn analyze_rust_modules(rust_files: &[RustBlobInfo]) -> Vec<RustModuleAnalysis> {
    ...
    for (module_path, files) in module_groups {
        ...
        // Find largest file
        let largest_file = files.iter().max_by_key(|f| f.blob_size).unwrap();
...
fn generate_module_optimizations(_module_path: &str, files: &[&RustBlobInfo]) -> Vec<String> {
    ...
    let large_files: Vec<&&RustBlobInfo> = files
        .iter()
...
fn generate_rust_blob_report(
    ...
    if !modules.is_empty() {
        ...
        for module in sorted_modules.iter().take(5) {
            println!(
                ...
                module.module_path,
                module.file_count,
                module.total_blob_size as f64 / 1024.0
            );
            println!(
                "    Largest: {} ({} bytes)",
                module.largest_file, module.largest_file_size
            );
    ...
    let large_files: Vec<&RustBlobInfo> = rust_files
        .iter()
        .filter(|f| f.blob_size > 100 * 1024)
        .collect();

    if !large_files.is_empty() {
        performance_concerns.push(format!(
            "{} large files may impact compilation speed",
            large_files.len()
        ));
    }

    let huge_files: Vec<&RustBlobInfo> = rust_files
        .iter()
    ...
    if !huge_files.is_empty() {
        performance_concerns.push(format!(
            "{} huge files may impact IDE performance",
            huge_files.len()
        ));
    }

    // Generate optimization recommendations
    if total_blob_size > 10 * 1024 * 1024 {
        // 10 MB
        optimization_recommendations
            .push("Consider splitting large modules to improve compilation speed".to_string());
    }

    if large_files.len() > 10 {
        optimization_recommendations.push(
            "Many large files - consider refactoring for better incremental compilation"
                .to_string(),
        );
    }
    ...
    println!(
        "  • Total blob size: {:.2} MB",
        total_blob_size as f64 / (1024.0 * 1024.0)
    );
    println!(
        "  • Average blob size: {:.1} KB",
        total_blob_size as f64 / rust_files.len() as f64 / 1024.0
    );
