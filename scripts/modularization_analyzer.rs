...
fn analyze_modularization_candidates(
) -> Result<Vec<ModularizationCandidate>, Box<dyn std::error::Error>> {
    ...
    if parts.len() >= 2 {
        ...
        match extension {
            ".rs" => {
                if files.len() > 3 {
                    (
                        "Module Extraction".to_string(),
                        "Consider extracting common functionality into shared modules".to_string(),
                    )
                } else {
                    (
                        "Function Extraction".to_string(),
                        "Extract common functions into utility modules".to_string(),
                    )
                }
            }
            ".py" => (
                "Package Refactoring".to_string(),
                "Consider creating a shared package for common functionality".to_string(),
            ),
            ".js" | ".ts" => (
                "Component Extraction".to_string(),
                "Extract common components into shared libraries".to_string(),
            ),
            _ => (
                "Code Consolidation".to_string(),
                "Consider consolidating similar code patterns".to_string(),
            ),
        }
    } else {
        (
            "General Refactoring".to_string(),
            "Review for code duplication and modularization opportunities".to_string(),
        )
    }
}
...
fn analyze_delta_compression_groups(
) -> Result<Vec<DeltaCompressionGroup>, Box<dyn std::error::Error>> {
...
fn calculate_pattern_savings(files: &[String]) -> u64 {
    ...
    let total_size: u64 = files.iter().map(|f| get_file_size(f).unwrap_or(0)).sum();
...
fn generate_modularization_report(
    ...
    if !candidates.is_empty() {
        ...
        for candidate in candidates {
            type_groups
                .entry(candidate.modularization_type.clone())
                .or_insert_with(Vec::new)
                .push(candidate);
        }
        ...
        for (mod_type, candidates) in type_groups {
            ...
            for candidate in candidates.iter().take(3) {
                println!(
                    "    • {} (similarity: {:.2})",
                    candidate.path, candidate.similarity_score
                );
        ...
        recommendations
            .push("Consider creating utility libraries for repeated patterns".to_string());
    }
    ...
    if !code_patterns.is_empty() {
        ...
        recommendations
            .push("Consolidate similar code patterns into shared components".to_string());
    }
