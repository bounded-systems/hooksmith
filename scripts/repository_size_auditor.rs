...
fn get_git_directory_size() -> Result<u64, Box<dyn std::error::Error>> {
    let output = Command::new("du").args(&["-sh", ".git"]).output()?;
...
fn get_working_directory_size() -> Result<u64, Box<dyn std::error::Error>> {
    let output = Command::new("du").args(&["-sh", "."]).output()?;
...
fn check_size_thresholds(
    metrics: &RepoSizeMetrics,
) -> Result<Vec<SizeThreshold>, Box<dyn std::error::Error>> {
    let mut thresholds = Vec::new();

    // Git directory size threshold (300 MB)
    let git_size_mb = metrics.git_directory_size as f64 / (1024.0 * 1024.0);
    let git_status = if git_size_mb < 300.0 {
        ThresholdStatus::Healthy
    } else if git_size_mb < 500.0 {
    ...
    let working_status = if working_size_mb < 200.0 {
        ThresholdStatus::Healthy
    } else if working_size_mb < 500.0 {
        ThresholdStatus::Warning
    } else {
        ThresholdStatus::Critical
    };
    ...
    let files_status = if metrics.total_tracked_files < 5000 {
        ThresholdStatus::Healthy
    } else if metrics.total_tracked_files < 10000 {
        ThresholdStatus::Warning
    } else {
        ThresholdStatus::Critical
    };
    ...
    let largest_status = if largest_file_mb < 1.0 {
        ThresholdStatus::Healthy
    } else if largest_file_mb < 10.0 {
        ThresholdStatus::Warning
    } else {
        ThresholdStatus::Critical
    };
    ...
    thresholds.push(SizeThreshold {
        ...
        recommendation: if largest_file_mb > 1.0 {
            format!(
                "Consider Git LFS for large file: {}",
                metrics.largest_file_path
            )
        } else {
    ...
    let crate_status = if metrics.crate_count < 50 {
        ThresholdStatus::Healthy
    } else if metrics.crate_count < 100 {
        ThresholdStatus::Warning
    } else {
        ThresholdStatus::Critical
    };
    ...
    let depth_status = if metrics.deep_tree_depth < 10 {
        ThresholdStatus::Healthy
    } else if metrics.deep_tree_depth < 15 {
        ThresholdStatus::Warning
    } else {
        ThresholdStatus::Critical
    };
...
fn generate_size_recommendations(
    metrics: &RepoSizeMetrics,
    thresholds: &[SizeThreshold],
) -> Vec<String> {
    let mut recommendations = Vec::new();

    let critical_count = thresholds
        .iter()
        .filter(|t| matches!(t.status, ThresholdStatus::Critical))
        .count();
    let warning_count = thresholds
        .iter()
    ...
    if critical_count > 0 {
        recommendations
            .push("Critical size issues detected - immediate action required".to_string());
    }

    if warning_count > 3 {
        recommendations
            .push("Multiple size warnings - consider repository optimization".to_string());
    }
...
fn calculate_health_score(thresholds: &[SizeThreshold]) -> f64 {
    ...
    let healthy_count = thresholds
        .iter()
        .filter(|t| matches!(t.status, ThresholdStatus::Healthy))
        .count() as f64;
    let warning_count = thresholds
        .iter()
        .filter(|t| matches!(t.status, ThresholdStatus::Warning))
...
fn generate_optimization_plan(
    ...
    let critical_thresholds: Vec<_> = thresholds
        .iter()
        .filter(|t| matches!(t.status, ThresholdStatus::Critical))
        .collect();
    ...
    let warning_thresholds: Vec<_> = thresholds
        .iter()
        .filter(|t| matches!(t.status, ThresholdStatus::Warning))
        .collect();
