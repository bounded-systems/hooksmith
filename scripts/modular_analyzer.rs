...
fn run_blob_size_analysis() -> Result<AnalysisResult, Box<dyn std::error::Error>> {
    ...
    let summary = format!("Found {} files in the 8-200 KB sweet spot ({:.1}%)", 
...
fn run_file_type_analysis() -> Result<AnalysisResult, Box<dyn std::error::Error>> {
    ...
    let text_files = file_types.iter()
    ...
    let summary = format!("{} text files ({:.1}%) - good for delta compression", 
...
fn run_frequent_writes_analysis() -> Result<AnalysisResult, Box<dyn std::error::Error>> {
    ...
    let summary = format!("Found {} files that should be in .gitignore", total_problematic);
