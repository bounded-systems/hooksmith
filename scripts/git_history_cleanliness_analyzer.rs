...
fn analyze_git_history_cleanliness() -> Result<CleanlinessAnalysis, Box<dyn std::error::Error>> {
    ...
    let critical_issues = identify_critical_issues(
        &commit_quality,
        &blob_health,
        &subtree_readiness,
        &tree_stability,
...
fn analyze_commit_messages() -> Result<(u32, u32, u32), Box<dyn std::error::Error>> {
    ...
    for line in commits.lines() {
        ...
        if message.len() > 10
            ...
            && !message.to_lowercase().contains("fix bug")
        ...
        if message.to_lowercase().contains("wip")
            || message.to_lowercase().contains("work in progress")
        {
            wip_commits += 1;
        }
    }
...
fn analyze_blob_health() -> Result<BlobHealthAnalysis, Box<dyn std::error::Error>> {
    ...
    let deduplication_score =
        calculate_deduplication_score(total_blobs, duplicate_blobs, large_blobs);
...
fn check_immutable_after_release() -> Result<bool, Box<dyn std::error::Error>> {
    ...
    let tag_output = Command::new("git").args(&["tag"]).output()?;
...
fn count_release_tags() -> Result<u32, Box<dyn std::error::Error>> {
    let output = Command::new("git").args(&["tag"]).output()?;
...
fn identify_critical_issues(
    ...
    if commit_quality.quality_score < 0.3 {
        critical_issues
            .push("Critical: Poor commit quality affecting contract stability".to_string());
    }
    ...
    if tree_stability.stability_score < 0.3 {
        critical_issues
            .push("Critical: Unstable tree structure affecting SHA stability".to_string());
    }
...
fn generate_git_history_cleanliness_report(analysis: &CleanlinessAnalysis) {
    ...
    println!(
        "  • Total commits: {}",
        analysis.commit_quality.total_commits
    );
    println!(
        "  • Linear commits: {} ({:.1}%)",
        analysis.commit_quality.linear_commits,
        (analysis.commit_quality.linear_commits as f64
            / analysis.commit_quality.total_commits as f64)
            * 100.0
    );
    println!(
        "  • Conventional commits: {} ({:.1}%)",
        analysis.commit_quality.conventional_commits,
        (analysis.commit_quality.conventional_commits as f64
            / analysis.commit_quality.total_commits as f64)
            * 100.0
    );
    println!(
        "  • Squash opportunities: {}",
        analysis.commit_quality.squash_opportunities
    );
    println!(
        "  • Quality score: {:.1}%",
        analysis.commit_quality.quality_score * 100.0
    );
    ...
    println!(
        "  • Duplicate blobs: {}",
        analysis.blob_health.duplicate_blobs
    );
    println!("  • Large blobs: {}", analysis.blob_health.large_blobs);
    println!(
        "  • LFS candidates: {}",
        analysis.blob_health.lfs_candidates
    );
    println!(
        "  • Packfile efficiency: {:.1}%",
        analysis.blob_health.packfile_efficiency * 100.0
    );
    println!(
        "  • Deduplication score: {:.1}%",
        analysis.blob_health.deduplication_score * 100.0
    );
    ...
    println!(
        "  • Modular structure: {}",
        if analysis.subtree_readiness.modular_structure {
            "✅"
        } else {
            "❌"
        }
    ...
    println!(
        "  • Cross-crate imports: {}",
        analysis.subtree_readiness.cross_crate_imports
    );
    println!(
        "  • Stable boundaries: {}",
        analysis.subtree_readiness.stable_boundaries
    );
    println!(
        "  • Extractable crates: {}",
        analysis.subtree_readiness.extractable_crates.len()
    );
    println!(
        "  • Readiness score: {:.1}%",
        analysis.subtree_readiness.readiness_score * 100.0
    );
    ...
    println!(
        ...
        analysis.tree_stability.volatile_directories
    );
    println!(
        "  • Stable layout: {}",
        if analysis.tree_stability.stable_layout {
            "✅"
        } else {
            "❌"
        }
    ...
    println!(
        "  • Rehash frequency: {:.1}%",
        analysis.tree_stability.rehash_frequency * 100.0
    );
    println!(
        "  • Stability score: {:.1}%",
        analysis.tree_stability.stability_score * 100.0
    );
    ...
    println!(
        "  • Pinned contracts: {}",
        analysis.contract_safety.pinned_contracts
    );
    println!("  • Stable SHAs: {}", analysis.contract_safety.stable_shas);
    println!(
        "  • Immutable after release: {}",
        if analysis.contract_safety.immutable_after_release {
            "✅"
        } else {
            "❌"
        }
    ...
    println!(
        "  • Release tags: {}",
        analysis.contract_safety.release_tags
    );
    println!(
        "  • Safety score: {:.1}%",
        analysis.contract_safety.safety_score * 100.0
    );
