...
fn analyze_crate_stability() -> Result<CrateStabilityAnalysis, Box<dyn std::error::Error>> {
    ...
    crates.sort_by(|a, b| b.extraction_readiness.partial_cmp(&a.extraction_readiness).unwrap());
...
fn analyze_single_crate_stability(cargo_path: &str) -> Result<Option<CrateStabilityInfo>, Box<dyn std::error::Error>> {
...
fn calculate_change_velocity(git_commits: u32, recent_changes: u32) -> Result<f64, Box<dyn std::error::Error>> {
    if git_commits == 0 {
        return Ok(0.0);
    }
...
fn count_api_surface_changes(crate_path: &str) -> Result<u32, Box<dyn std::error::Error>> {
    ...
    for line in commits.lines() {
        if line.contains("pub") || line.contains("struct") || line.contains("enum") || line.contains("trait") {
            api_changes += 1;
        }
    }
...
fn calculate_api_stability_score(api_surface_changes: u32, git_commits: u32) -> Result<f64, Box<dyn std::error::Error>> {
    if git_commits == 0 {
        return Ok(1.0);
    }
...
fn calculate_coupling_score(dependent_crates: &[String]) -> Result<f64, Box<dyn std::error::Error>> {
...
fn determine_extraction_readiness(
    ...
    if change_velocity < 0.3 && api_stability_score > 0.7 && dependent_crates.len() > 2 && git_commits > 10 {
...
fn generate_stabilization_plan(crates: &[CrateStabilityInfo]) -> Vec<String> {
    ...
    let needs_stabilization: Vec<_> = crates.iter()
        .filter(|c| matches!(c.extraction_readiness, ExtractionReadiness::NeedsStabilization))
        .collect();
    ...
    let too_volatile: Vec<_> = crates.iter()
...
fn generate_external_repo_strategy(crates: &[CrateStabilityInfo]) -> Vec<String> {
    ...
    let ready_crates: Vec<_> = crates.iter()
        .filter(|c| matches!(c.extraction_readiness, ExtractionReadiness::Ready))
        .collect();
    ...
    strategy.push("  git-utils = { git = \"https://github.com/hooksmith/git-utils\", tag = \"v1.0.0\" }".to_string());
...
fn calculate_overall_stability_score(crates: &[CrateStabilityInfo]) -> f64 {
    ...
    let total_score: f64 = crates.iter()
        .map(|c| {
...
fn generate_crate_stability_report(analysis: &CrateStabilityAnalysis) {
    ...
    let ready_count = analysis.crates.iter().filter(|c| matches!(c.extraction_readiness, ExtractionReadiness::Ready)).count();
    let needs_stabilization_count = analysis.crates.iter().filter(|c| matches!(c.extraction_readiness, ExtractionReadiness::NeedsStabilization)).count();
    let too_volatile_count = analysis.crates.iter().filter(|c| matches!(c.extraction_readiness, ExtractionReadiness::TooVolatile)).count();
