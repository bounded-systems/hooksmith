...
fn analyze_file_stability(path: &str) -> Result<Option<ContractStabilityIssue>, Box<dyn std::error::Error>> {
    // Get file size
    ...
    let description = generate_issue_description(&issues, object_size, change_frequency, sha_churn_score);
...
fn get_file_content_at_commit(path: &str, commit: &str) -> Result<String, Box<dyn std::error::Error>> {
    let output = Command::new("git")
...
fn determine_severity(issues: &[StabilityIssueType], size: u64, frequency: u32, churn: f64) -> IssueSeverity {
...
fn generate_issue_description(issues: &[StabilityIssueType], size: u64, frequency: u32, churn: f64) -> String {
    ...
    for issue in issues {
        match issue {
            ...
            StabilityIssueType::UnstableContract => {
                descriptions.push("Unstable contract - frequent changes with high churn".to_string());
            }
...
fn analyze_modular_boundaries(issues: &[ContractStabilityIssue]) -> HashMap<String, Vec<String>> {
    ...
    for issue in issues {
        ...
        boundaries.entry(module).or_insert_with(Vec::new).push(issue.file_path.clone());
    }
...
fn calculate_stability_score(issues: &[ContractStabilityIssue]) -> f64 {
    ...
    let critical_issues = issues.iter().filter(|i| matches!(i.severity, IssueSeverity::Critical)).count() as f64;
    let high_issues = issues.iter().filter(|i| matches!(i.severity, IssueSeverity::High)).count() as f64;
    let medium_issues = issues.iter().filter(|i| matches!(i.severity, IssueSeverity::Medium)).count() as f64;
    
    let weighted_score = (critical_issues * 4.0 + high_issues * 2.0 + medium_issues * 1.0) / total_issues;
    
    (1.0 - weighted_score).max(0.0)
}

fn generate_stability_recommendations(issues: &[ContractStabilityIssue], sha_churn: &HashMap<String, f64>) -> Vec<String> {
    let mut recommendations = Vec::new();
    ...
    if critical_count > 0 {
        recommendations.push("Critical stability issues detected - prioritize contract optimization".to_string());
    }
    
    if high_count > 5 {
        recommendations.push("Multiple high-severity issues - implement contract memoization".to_string());
    }
    ...
    let large_blobs = issues.iter().filter(|i| matches!(i.issue_type, StabilityIssueType::LargeBlob)).count();
    if large_blobs > 3 {
        recommendations.push("Multiple large blobs - implement modularization strategy".to_string());
    }
...
fn generate_contract_stability_report(analysis: &ContractStabilityAnalysis) {
    ...
    let critical_issues: Vec<_> = analysis.issues.iter()
        .filter(|i| matches!(i.severity, IssueSeverity::Critical))
        .collect();
    
    let high_issues: Vec<_> = analysis.issues.iter()
    ...
    if !high_issues.is_empty() {
        ...
        if high_issues.len() > 5 {
            println!("  ... and {} more high-severity issues", high_issues.len() - 5);
        }
    }
    
    // Show stability score
    println!("\n📊 Stability Metrics:");
    println!("  • Overall stability score: {:.1}%", analysis.stability_score * 100.0);
    ...
    if !analysis.sha_churn_analysis.is_empty() {
        let avg_churn: f64 = analysis.sha_churn_analysis.values().sum::<f64>() / analysis.sha_churn_analysis.len() as f64;
        println!("  • Average SHA churn: {:.1}%", avg_churn * 100.0);
        
        let high_churn_files: Vec<_> = analysis.sha_churn_analysis.iter()
    ...
    if !analysis.modular_boundaries.is_empty() {
        ...
        for (module, files) in &analysis.modular_boundaries {
            if files.len() > 1 {
                println!("  • {}: {} files with stability issues", module, files.len());
            }
        }
    }
