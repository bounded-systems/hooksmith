...
fn analyze_tree_object_stability(time_period: Option<&str>) -> Result<TreeStabilityAnalysis, Box<dyn std::error::Error>> {
    ...
    let structural_decomposition_targets = identify_structural_decomposition_targets(&unstable_objects);
...
fn get_object_stability_data(time_period: Option<&str>) -> Result<Vec<ObjectStabilityInfo>, Box<dyn std::error::Error>> {
...
fn analyze_single_object(object_path: &str, commit_count: u32, time_period: Option<&str>) -> Result<ObjectStabilityInfo, Box<dyn std::error::Error>> {
    ...
    let delta_compression_impact = calculate_delta_compression_impact(commit_count, total_size, blob_count);
    
    // Calculate contract risk score
    let contract_risk_score = calculate_contract_risk_score(commit_count, &stability_level, concurrent_editors.len());
    
    // Generate recommendations
    let recommendations = generate_stability_recommendations(object_path, commit_count, &stability_level, &object_type, tree_depth, blob_count);
...
fn calculate_churn_score(commit_count: u32, object_type: &ObjectType, tree_depth: u32, blob_count: u32) -> f64 {
...
fn estimate_merge_conflicts(object_path: &str, commit_count: u32) -> Result<u32, Box<dyn std::error::Error>> {
...
fn get_concurrent_editors(object_path: &str, time_period: Option<&str>) -> Result<Vec<String>, Box<dyn std::error::Error>> {
...
fn calculate_contract_risk_score(commit_count: u32, stability_level: &StabilityLevel, concurrent_editors: usize) -> f64 {
...
fn generate_stability_recommendations(object_path: &str, commit_count: u32, stability_level: &StabilityLevel, object_type: &ObjectType, tree_depth: u32, blob_count: u32) -> Vec<String> {
...
fn identify_high_churn_files(unstable_objects: &[ObjectStabilityInfo]) -> Vec<String> {
    unstable_objects.iter()
        .filter(|obj| matches!(obj.stability_level, StabilityLevel::Critical | StabilityLevel::High))
...
fn identify_structural_decomposition_targets(unstable_objects: &[ObjectStabilityInfo]) -> Vec<String> {
    unstable_objects.iter()
        .filter(|obj| {
...
fn calculate_stability_stats(unstable_objects: &[ObjectStabilityInfo]) -> StabilityStats {
    ...
    let critical_instability = unstable_objects.iter().filter(|obj| matches!(obj.stability_level, StabilityLevel::Critical)).count() as u32;
    let high_instability = unstable_objects.iter().filter(|obj| matches!(obj.stability_level, StabilityLevel::High)).count() as u32;
    let medium_instability = unstable_objects.iter().filter(|obj| matches!(obj.stability_level, StabilityLevel::Medium)).count() as u32;
    let low_instability = unstable_objects.iter().filter(|obj| matches!(obj.stability_level, StabilityLevel::Low)).count() as u32;
    let stable_objects = unstable_objects.iter().filter(|obj| matches!(obj.stability_level, StabilityLevel::Stable)).count() as u32;
    
    let merge_conflict_risk = unstable_objects.iter().filter(|obj| obj.merge_conflicts > 5).count() as u32;
    let delta_compression_impact = unstable_objects.iter().map(|obj| obj.delta_compression_impact).sum::<f64>() / total_objects as f64;
    let contract_risk_score = unstable_objects.iter().map(|obj| obj.contract_risk_score).sum::<f64>() / total_objects as f64;
...
fn generate_stability_audit_report(analysis: &TreeStabilityAnalysis) {
    ...
    println!("  • Total objects analyzed: {}", analysis.overall_stats.total_objects);
    println!("  • Critical instability: {}", analysis.overall_stats.critical_instability);
    println!("  • High instability: {}", analysis.overall_stats.high_instability);
    println!("  • Medium instability: {}", analysis.overall_stats.medium_instability);
    println!("  • Low instability: {}", analysis.overall_stats.low_instability);
    println!("  • Stable objects: {}", analysis.overall_stats.stable_objects);
    println!("  • Merge conflict risk: {}", analysis.overall_stats.merge_conflict_risk);
    println!("  • Delta compression impact: {:.2}", analysis.overall_stats.delta_compression_impact);
    println!("  • Contract risk score: {:.2}", analysis.overall_stats.contract_risk_score);
    
    // Top unstable objects
    println!("\n🚨 Top 10 Most Unstable Objects:");
    for (i, obj) in analysis.unstable_objects.iter().take(10).enumerate() {
        println!("  {}. {} ({} commits, {:?}, {:.1} churn score)", 
            i + 1, obj.object_path, obj.commit_count, obj.stability_level, obj.churn_score);
    }
    ...
    if analysis.overall_stats.critical_instability > 0 {
        println!("  🚨 Critical instability detected - immediate structural decomposition required");
    }
