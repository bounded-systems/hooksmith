...
fn analyze_crate_granularity() -> Result<CrateGranularityAnalysis, Box<dyn std::error::Error>> {
    ...
    crates.sort_by(|a, b| b.granularity_score.partial_cmp(&a.granularity_score).unwrap());
...
fn count_binaries_and_libraries(crate_dir: &Path) -> Result<(u32, u32), Box<dyn std::error::Error>> {
    ...
    if src_dir.exists() {
        ...
        let bin_output = Command::new("find")
            .args(&[src_dir.to_str().unwrap(), "-name", "*.rs", "-path", "*/bin/*"])
            .output()?;
        ...
        let lib_output = Command::new("find")
            .args(&[src_dir.to_str().unwrap(), "-name", "lib.rs", "-o", "-name", "mod.rs"])
            .output()?;
...
fn generate_granularity_recommendations(crates: &[CrateAnalysis]) -> Vec<String> {
    ...
    recommendations.push("Aim for 100-800 LOC per crate for optimal contract granularity".to_string());
...
fn generate_contract_optimization(crates: &[CrateAnalysis]) -> Vec<String> {
    ...
    optimizations.push(format!("Total contracts: {} across {} crates", total_contracts, total_crates));
    
    if total_contracts > total_crates {
        optimizations.push("Multiple contracts per crate - optimize for single responsibility".to_string());
    }
...
fn generate_crate_granularity_report(analysis: &CrateGranularityAnalysis) {
    ...
    let optimal_count = analysis.crates.iter().filter(|c| c.granularity_score > 0.7).count();
    let large_count = analysis.crates.iter().filter(|c| c.line_count > 1500).count();
