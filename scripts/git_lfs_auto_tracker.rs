...
fn analyze_lfs_candidates() -> Result<LfsAnalysis, Box<dyn std::error::Error>> {
    ...
    if duplicate_count > 5 {
        recommendations
            .push("Multiple duplicate files - LFS can help with deduplication".to_string());
    }
...
fn should_consider_for_lfs(path: &str, size: u64) -> bool {
    ...
    let skip_extensions = [
        ".md", ".txt", ".json", ".yml", ".yaml", ".toml", ".rs", ".py", ".js", ".ts", ".sh",
        ".bash",
    ];
...
fn generate_lfs_report(analysis: &LfsAnalysis) {
    ...
    println!(
        "  • Binary files: {}",
        analysis.candidates.iter().filter(|c| c.is_binary).count()
    );
    println!(
        "  • Duplicate files: {}",
        analysis
    ...
    println!(
        ...
        analysis.total_savings as f64 / (1024.0 * 1024.0)
    );
    ...
    if analysis.candidates.is_empty() {
        ...
        println!(
            "  • {} files recommended for LFS tracking",
