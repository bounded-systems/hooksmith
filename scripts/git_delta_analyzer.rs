        ...
        if parts.len() >= 4 {
            ...
            if let Ok(size_str) = String::from_utf8(size_output.stdout) {
                if let Ok(blob_size) = u64::from_str(size_str.trim()) {
                    ...
                    if blob_size > 1024 && blob_size < 1024 * 1024 { // 1KB to 1MB
                        let similarity_score = calculate_similarity_score(&path, blob_size);
                        
                        if similarity_score > 0.3 { // Only consider files with reasonable similarity potential
...
fn calculate_similarity_score(path: &str, blob_size: u64) -> f64 {
    ...
    } else if path_lower.ends_with(".json") || path_lower.ends_with(".yml") || path_lower.ends_with(".yaml") {
...
fn analyze_delta_compression_insights(groups: &[DeltaGroup]) -> Vec<String> {
    ...
    insights.push(format!("Found {} delta groups with {} files", groups.len(), total_files));
    insights.push(format!("Total potential savings: {:.2} KB", total_savings as f64 / 1024.0));
    ...
    let avg_compression = groups.iter()
        .map(|g| g.compression_ratio)
        .sum::<f64>() / groups.len() as f64;
    
    insights.push(format!("Average compression ratio: {:.1}%", avg_compression * 100.0));
...
fn generate_delta_report(candidates: &[DeltaCandidate], groups: &[DeltaGroup]) -> DeltaAnalysis {
    ...
    for candidate in sorted_candidates.iter().take(10) {
        println!("  • {} ({} bytes, {:.1}% similarity)", 
            candidate.file_path, candidate.blob_size, candidate.similarity_score * 100.0);
    }
    ...
    if !groups.is_empty() {
        ...
        for group in sorted_groups.iter().take(5) {
            ...
            println!("    Savings: {:.1} KB ({:.1}% compression)", 
                group.total_savings as f64 / 1024.0, group.compression_ratio * 100.0);
    ...
    if !groups.is_empty() {
        let avg_group_size = groups.iter().map(|g| g.delta_files.len()).sum::<usize>() as f64 / groups.len() as f64;
