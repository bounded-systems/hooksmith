...
fn analyze_repository_state() -> Result<RepositoryState, Box<dyn std::error::Error>> {
    ...
    let branches_output = Command::new("git").args(&["branch", "-r"]).output()?;
    let branches_str = String::from_utf8(branches_output.stdout)?;
    let total_branches = branches_str.lines().count() as u32;

    // Get remote branches
    let remote_branches: Vec<String> = branches_str.lines().map(|s| s.trim().to_string()).collect();
...
) -> Result<RewriteOperation, Box<dyn std::error::Error>> {
...
fn create_rollback_plan(state: &RepositoryState) -> RollbackPlan {
    ...
    let restore_commands = vec![
        ...
        format!(
            "git push --force {} {}",
            backup_remote, state.current_branch
        ),
    ];
...
fn analyze_contract_implications_for_plan(
    state: &RepositoryState,
    target_commits: &[String],
) -> Vec<String> {
...
fn generate_git_history_rewrite_report(rewriter: &GitHistoryRewriter) {
    ...
    println!(
        "  • Current branch: {}",
        rewriter.current_state.current_branch
    );
    println!(
        "  • Total commits: {}",
        rewriter.current_state.total_commits
    );
    println!(
        "  • Total branches: {}",
        rewriter.current_state.total_branches
    );
    println!(
        "  • Active contracts: {}",
        rewriter.current_state.active_contracts.len()
    );
    ...
    println!(
        "  • Operation type: {:?}",
        rewriter.rewrite_plan.operation_type
    );
    println!(
        "  • Target commits: {}",
        rewriter.rewrite_plan.target_commits.len()
    );
    println!(
        "  • Affected files: {}",
        rewriter.rewrite_plan.affected_files.len()
    );
    println!(
        "  • Estimated impact: {:?}",
        rewriter.rewrite_plan.estimated_impact
    );
    ...
    println!(
        ...
        rewriter.safety_analysis.overall_safety_score * 100.0
    );

    let passed_checks = rewriter
        .safety_analysis
        .checks
        .iter()
        .filter(|c| matches!(c.status, CheckStatus::Passed))
        .count();
    let warning_checks = rewriter
        .safety_analysis
        .checks
        .iter()
        .filter(|c| matches!(c.status, CheckStatus::Warning))
        .count();
    let failed_checks = rewriter
        .safety_analysis
        .checks
        .iter()
        .filter(|c| matches!(c.status, CheckStatus::Failed))
        .count();
    let critical_checks = rewriter
        .safety_analysis
        .checks
        .iter()
        .filter(|c| matches!(c.status, CheckStatus::Critical))
    ...
    println!(
        "  • Affected contracts: {}",
        rewriter.contract_analysis.affected_contracts.len()
    );
    println!(
        "  • Contract hashes: {}",
        rewriter.contract_analysis.contract_hashes.len()
    );
    println!(
        "  • Invalidation risk: {:.1}%",
        rewriter.contract_analysis.invalidation_risk * 100.0
    );
    ...
    println!(
        "  • Backup branch: {}",
        rewriter.rewrite_plan.rollback_plan.backup_branch
    );
    println!(
        "  • Backup remote: {}",
        rewriter.rewrite_plan.rollback_plan.backup_remote
    );
