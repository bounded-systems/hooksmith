...
fn analyze_single_tree(
    tree_path: &str,
    files: &[String],
) -> Result<TreeStabilityInfo, Box<dyn std::error::Error>> {
...
fn analyze_tree_churn(
    tree_path: &str,
) -> Result<Option<TreeChurnAnalysis>, Box<dyn std::error::Error>> {
...
fn generate_memoization_recommendations(trees: &[TreeStabilityInfo]) -> Vec<String> {
    ...
    let stable_trees = trees
        .iter()
        .filter(|t| matches!(t.stability_level, TreeStabilityLevel::Stable))
        .count();
    let volatile_trees = trees
        .iter()
        .filter(|t| {
            matches!(
                t.stability_level,
                TreeStabilityLevel::Volatile | TreeStabilityLevel::Critical
            )
        })
        .count();
    ...
    if stable_trees > 5 {
        recommendations
            .push("Multiple stable trees detected - implement tree SHA memoization".to_string());
    }
        ...
        match tree.stability_level {
            TreeStabilityLevel::Stable => {
                recommendations.push(format!(
                    "Memoize fix plans for tree: {} (SHA: {})",
                    tree.tree_path, tree.tree_sha
                ));
            }
            TreeStabilityLevel::SemiVolatile => {
                recommendations.push(format!(
                    "Use conditional memoization for tree: {}",
                    tree.tree_path
                ));
            }
            TreeStabilityLevel::Volatile => {
                recommendations.push(format!(
                    "Isolate volatile files in tree: {} to prevent cascade",
                    tree.tree_path
                ));
            }
            TreeStabilityLevel::Critical => {
                recommendations.push(format!(
                    "Critical tree: {} - implement aggressive isolation",
                    tree.tree_path
                ));
            }
        }
    ...
    for tree in trees {
        if tree.volatile_files.len() > tree.stable_files.len() {
            optimizations.push(format!(
                "Split volatile files from tree: {} into separate subtree",
                tree.tree_path
            ));
        }
        ...
        if tree.file_count > 20 {
            optimizations.push(format!(
                "Large tree: {} - consider splitting into smaller subtrees",
                tree.tree_path
            ));
        }
    }
...
fn calculate_overall_stability_score(trees: &[TreeStabilityInfo]) -> f64 {
    ...
    let stable_trees = trees
        ...
        .filter(|t| matches!(t.stability_level, TreeStabilityLevel::Stable))
        .count() as f64;
    let critical_trees = trees
        .iter()
        .filter(|t| matches!(t.stability_level, TreeStabilityLevel::Critical))
        .count() as f64;
