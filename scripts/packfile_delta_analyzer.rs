...
fn analyze_packfile_deltas() -> Result<PackfileAnalysis, Box<dyn std::error::Error>> {
    ...
    let average_chain_length = if !delta_chains.is_empty() {
        delta_chains.iter().map(|chain| chain.chain_length as f64).sum::<f64>() / delta_chains.len() as f64
    } else {
        0.0
    };
    ...
    if average_chain_length > 50.0 {
        recommendations.push("High average delta chain length - consider repacking with --depth=50".to_string());
    }
    
    if savings_percentage < 30.0 {
        recommendations.push("Low compression ratio - consider repacking with --window=250".to_string());
    }
    
    if delta_chains.is_empty() {
        recommendations.push("No delta chains found - run 'git repack -Ad' to create deltas".to_string());
    }
...
fn generate_packfile_report(analysis: &PackfileAnalysis) {
    ...
    println!("  • Average chain length: {:.1}", analysis.statistics.average_chain_length);
    println!("  • Total compressed size: {:.2} MB", 
        analysis.statistics.total_compressed_size as f64 / (1024.0 * 1024.0));
    println!("  • Total uncompressed size: {:.2} MB", 
        analysis.statistics.total_uncompressed_size as f64 / (1024.0 * 1024.0));
    println!("  • Overall compression ratio: {:.1}%", 
        (1.0 - analysis.statistics.overall_compression_ratio) * 100.0);
    println!("  • Delta savings: {:.2} MB ({:.1}%)", 
    ...
    if !analysis.delta_chains.is_empty() {
        ...
        for (i, chain) in sorted_chains.iter().take(5).enumerate() {
            println!("  {}. Base: {} (chain length: {})", i + 1, chain.base_object, chain.chain_length);
            println!("     Compression: {:.1}% ({} → {} bytes)", 
    ...
    if analysis.statistics.total_objects > 0 {
        ...
        println!("  • Delta compression: {:.1}% effective", analysis.statistics.savings_percentage);
        
        if analysis.statistics.average_chain_length > 0.0 {
            println!("  • Average delta chain: {:.1} objects", analysis.statistics.average_chain_length);
        }
        
        if analysis.statistics.delta_savings > 0 {
            println!("  • Total savings: {:.2} MB", 
                analysis.statistics.delta_savings as f64 / (1024.0 * 1024.0));
        }
    } else {
