    ...
    fn audit(&self, report: &[Value], mandate: &[Value]) -> Result<Vec<Value>> {
        ...
        for item in mandate {
            if let (Some(item_type), Some(name)) = (item["type"].as_str(), item["name"].as_str()) {
                match item_type {
                    "required" => { required.insert(name.to_string()); }
                    "allowed" => { allowed.insert(name.to_string()); }
                    "rejected" => { rejected.insert(name.to_string()); }
                    "ignored" => { ignored.insert(name.to_string()); }
                    _ => {}
                }
            }
        }
...
impl TriageOfficer {
    ...
    fn create_sarif(&self, diff: &[Value], tree_sha: &str) -> Result<Value> {
        ...
        for violation in diff {
            if let (Some(kind), Some(name)) = (violation["kind"].as_str(), violation["name"].as_str()) {
