...
fn analyze_modularization_packing_impact() -> Result<ModularizationPackingAnalysis, Box<dyn std::error::Error>> {
    ...
    for line in files_output.lines() {
        ...
        if !path.is_empty() {
            ...
            module_groups.entry(module_path).or_default().push((path.to_string(), size));
        }
    }
    ...
    modules.sort_by(|a, b| b.modularization_score.partial_cmp(&a.modularization_score).unwrap());
...
fn analyze_module(module_path: &str, files: &[(String, u64)]) -> Result<ModuleAnalysis, Box<dyn std::error::Error>> {
    let file_count = files.len() as u32;
    let total_size: u64 = files.iter().map(|(_, size)| size).sum();
    let average_size = if file_count > 0 { total_size / file_count as u64 } else { 0 };
    ...
    let modularization_score = calculate_modularization_score(file_count, average_size, shared_patterns.len(), delta_potential);
...
fn calculate_delta_potential(files: &[(String, u64)]) -> f64 {
    ...
    for i in 0..files.len() {
        for j in (i + 1)..files.len() {
            let size_diff = (files[i].1 as f64 - files[j].1 as f64).abs() / files[i].1.max(files[j].1) as f64;
            
            // Similar sized files have higher delta potential
            if size_diff < 0.3 { // Within 30% size difference
...
fn calculate_modularization_score(file_count: u32, average_size: u64, shared_patterns: usize, delta_potential: f64) -> f64 {
    let mut score = 0.0;
    ...
    // Size factor (0-0.2)
    if average_size > 50 * 1024 { // 50KB+
        score += 0.2;
    } else if average_size > 10 * 1024 { // 10KB+
        score += 0.1;
    }
...
fn calculate_packing_impact(modules: &[ModuleAnalysis]) -> Result<PackingImpact, Box<dyn std::error::Error>> {
    ...
    let improvement_estimate = if current_metrics.compression_ratio > 0.0 {
        (improved_metrics.compression_ratio - current_metrics.compression_ratio) / current_metrics.compression_ratio * 100.0
    } else {
        0.0
    };
    ...
    if improvement_estimate > 20.0 {
        recommendations.push("High potential for packing improvement through modularization".to_string());
    }
    
    let large_modules = modules.iter().filter(|m| m.modularization_score > 0.7).count();
    if large_modules > 5 {
    ...
    if high_delta_potential > 3 {
        recommendations.push("High delta compression potential - modularization will improve packing".to_string());
    }
...
fn calculate_current_packing_metrics(modules: &[ModuleAnalysis]) -> Result<PackingMetrics, Box<dyn std::error::Error>> {
...
fn estimate_improved_packing_metrics(modules: &[ModuleAnalysis], current: &PackingMetrics) -> PackingMetrics {
    // Estimate improvements based on modularization opportunities
    let improvement_factor = modules.iter()
        .filter(|m| m.modularization_score > 0.5)
        .map(|m| m.modularization_score)
        .sum::<f64>() / modules.len().max(1) as f64;
    
    let improved_compression_ratio = current.compression_ratio * (1.0 - improvement_factor * 0.3);
    let improved_savings = ((current.total_objects as u64 * 1024) as f64 * (1.0 - improved_compression_ratio)) as u64;
    
    PackingMetrics {
        total_objects: current.total_objects,
        delta_chains: current.delta_chains + modules.iter().filter(|m| m.modularization_score > 0.7).count() as u32,
...
fn generate_implementation_plan(modules: &[ModuleAnalysis]) -> Vec<String> {
    ...
    let high_priority = modules.iter().filter(|m| m.modularization_score > 0.8).collect::<Vec<_>>();
    if !high_priority.is_empty() {
        plan.push("Phase 1 - High Priority Modules:".to_string());
        for module in high_priority {
            plan.push(format!("  • Extract shared logic from '{}'", module.module_path));
        }
    }
    
    // Medium priority modules
    let medium_priority = modules.iter().filter(|m| m.modularization_score > 0.6 && m.modularization_score <= 0.8).collect::<Vec<_>>();
    if !medium_priority.is_empty() {
        plan.push("Phase 2 - Medium Priority Modules:".to_string());
        for module in medium_priority {
            plan.push(format!("  • Refactor '{}' for better delta compression", module.module_path));
        }
    }
...
fn generate_modularization_report(analysis: &ModularizationPackingAnalysis) {
    ...
    for (i, module) in analysis.modules.iter().take(10).enumerate() {
        println!("  {}. {} (Score: {:.1})", i + 1, module.module_path, module.modularization_score);
        println!("     Files: {} | Avg Size: {:.1}KB | Delta Potential: {:.1}", 
            module.file_count, module.average_size as f64 / 1024.0, module.delta_potential);
    ...
    println!("    • Total objects: {}", analysis.packing_impact.before_modularization.total_objects);
    println!("    • Delta chains: {}", analysis.packing_impact.before_modularization.delta_chains);
    println!("    • Compression ratio: {:.1}%", 
        (1.0 - analysis.packing_impact.before_modularization.compression_ratio) * 100.0);
    println!("    • Estimated savings: {:.2} MB", 
        analysis.packing_impact.before_modularization.estimated_savings as f64 / (1024.0 * 1024.0));
    
    println!("  After Modularization:");
    println!("    • Total objects: {}", analysis.packing_impact.after_modularization.total_objects);
    println!("    • Delta chains: {}", analysis.packing_impact.after_modularization.delta_chains);
    println!("    • Compression ratio: {:.1}%", 
        (1.0 - analysis.packing_impact.after_modularization.compression_ratio) * 100.0);
    println!("    • Estimated savings: {:.2} MB", 
        analysis.packing_impact.after_modularization.estimated_savings as f64 / (1024.0 * 1024.0));
    
    println!("  Improvement: {:.1}%", analysis.packing_impact.improvement_estimate);
    ...
    println!("  • {} high-priority modules for modularization", 
        analysis.modules.iter().filter(|m| m.modularization_score > 0.7).count());
    println!("  • Estimated packing improvement: {:.1}%", analysis.packing_impact.improvement_estimate);
