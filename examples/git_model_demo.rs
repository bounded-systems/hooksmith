...
fn main() {
    ...
    config.commands.insert("rustfmt".to_string(), "cargo fmt --all -- --check".to_string());
    config.commands.insert("clippy".to_string(), "cargo clippy --all-targets --all-features -- -D warnings".to_string());
    config.skip = Some(SkipCondition::Conditions(vec!["CI".to_string()]));

    let files_with_config = vec![
        ("src/main.rs".to_string(), FileStateKind::Staged, Some(config.clone())),
        ("src/lib.rs".to_string(), FileStateKind::Staged, Some(config.clone())),
    ...
    for file_info in file_analyses_with_config {
        println!("  File: {}", file_info.path);
        println!("    State: {:?}", file_info.state);
        println!("    Has Lefthook config: {}", if file_info.lefthook_config.is_some() { "✅ Yes" } else { "❌ No" });

        if let Some(config) = &file_info.lefthook_config {
            println!("    File pattern: {}", config.files.as_deref().unwrap_or("None"));
            println!("    Parallel: {}", config.parallel.map(|p| if p { "Yes" } else { "No" }).unwrap_or_else(|| "Not set"));
            println!("    Commands: {}", config.commands.keys().cloned().collect::<Vec<_>>().join(", "));
            println!("    Matches pattern: {}", if file_info.matches_file_pattern() { "✅ Yes" } else { "❌ No" });
            println!("    Should skip: {}", if file_info.should_skip() { "✅ Yes" } else { "❌ No" });
            println!("    Should run: {}", if file_info.should_run() { "✅ Yes" } else { "❌ No" });
        }
    }
    ...
    rust_config.commands.insert("fmt".to_string(), "cargo fmt --all -- --check".to_string());
    rust_config.commands.insert("clippy".to_string(), "cargo clippy --all-targets --all-features -- -D warnings".to_string());
    rust_config.commands.insert("test".to_string(), "cargo test --all-targets --all-features".to_string());

    println!("  Rust Project Config:");
    println!("    Files: {}", rust_config.files.as_deref().unwrap_or("None"));
    println!("    Parallel: {}", rust_config.parallel.map(|p| if p { "Yes" } else { "No" }).unwrap_or_else(|| "Not set"));
    println!("    Commands: {}", rust_config.commands.keys().cloned().collect::<Vec<_>>().join(", "));
    ...
    ts_config.commands.insert("lint".to_string(), "npm run lint".to_string());
    ts_config.commands.insert("type-check".to_string(), "npm run type-check".to_string());
    ts_config.skip = Some(SkipCondition::Conditions(vec!["WIP".to_string()]));

    println!("  TypeScript Project Config:");
    println!("    Files: {}", ts_config.files.as_deref().unwrap_or("None"));
    println!("    Parallel: {}", ts_config.parallel.map(|p| if p { "Yes" } else { "No" }).unwrap_or_else(|| "Not set"));
    println!("    Commands: {}", ts_config.commands.keys().cloned().collect::<Vec<_>>().join(", "));
    ...
    rustfmt_cmd.env.insert("RUST_BACKTRACE".to_string(), "1".to_string());
    rustfmt_cmd.fail_text = Some("Code formatting check failed. Run 'cargo fmt' to fix.".to_string());
    rustfmt_cmd.stage_fixed = Some(true);

    println!("  Rustfmt Command:");
    println!("    Run: {}", rustfmt_cmd.run);
    println!("    Files: {}", rustfmt_cmd.files.as_deref().unwrap_or("None"));
    ...
    println!("    Fail Text: {}", rustfmt_cmd.fail_text.as_deref().unwrap_or("None"));
    println!("    Stage Fixed: {}", rustfmt_cmd.stage_fixed.unwrap_or(false));
    println!("    Matches src/main.rs: {}", rustfmt_cmd.matches_file("src/main.rs"));
    println!("    Matches src/main.py: {}", rustfmt_cmd.matches_file("src/main.py"));
    ...
    ts_lint_cmd.exclude = Some(ExcludeCondition::Patterns(vec!["*.test.ts".to_string(), "*.spec.ts".to_string()]));
    ...
    println!("    Interactive: {}", ts_lint_cmd.interactive.unwrap_or(false));
    println!("    Use Stdin: {}", ts_lint_cmd.use_stdin.unwrap_or(false));
    println!("    Matches src/components/Button.tsx: {}", ts_lint_cmd.matches_file("src/components/Button.tsx"));
    println!("    Matches src/components/Button.test.ts: {}", ts_lint_cmd.matches_file("src/components/Button.test.ts"));
    ...
    println!("    Parallel: {}", advanced_config.parallel.map(|p| if p { "Yes" } else { "No" }).unwrap_or_else(|| "Not set"));
    println!("    Files: {}", advanced_config.files.as_deref().unwrap_or("None"));
    ...
    println!("    All File Types: {}", advanced_config.all_file_types().join(", "));
    println!("    Has Incompatible File Types: {}", advanced_config.has_incompatible_file_types());

    // Test commands for specific files
    println!("    Commands for src/main.rs: {}", advanced_config.commands_for_file("src/main.rs").len());
    println!("    Commands for src/components/Button.tsx: {}", advanced_config.commands_for_file("src/components/Button.tsx").len());
    println!("    Commands for src/components/Button.test.ts: {}", advanced_config.commands_for_file("src/components/Button.test.ts").len());
    ...
    let context = FileSubstitutionContext::new(
        ...
        vec!["src/main.rs".to_string(), "src/lib.rs".to_string(), "docs/README.md".to_string(), "Cargo.toml".to_string()],
    ...
    for command in compatible_commands {
        println!("    {}: {}", command, if is_run_files_compatible(command) { "✅ Compatible" } else { "❌ Incompatible" });
    }
    ...
    let mut incompatible_cmd = LefthookCommand::new("cargo fmt {staged_files} {push_files}".to_string());

    println!("  Rustfmt Command:");
    println!("    Run: {}", fmt_cmd.run);
    println!("    Uses file substitutions: {}", fmt_cmd.uses_file_substitutions());
    println!("    Is compatible: {}", fmt_cmd.is_files_compatible());
    println!("    Resolved: {}", fmt_cmd.get_resolved_command(&context));
    println!("    Used substitutions: {}", fmt_cmd.get_used_substitutions(&context).join(", "));

    println!("  Lint Command:");
    println!("    Run: {}", lint_cmd.run);
    println!("    Uses file substitutions: {}", lint_cmd.uses_file_substitutions());
    println!("    Is compatible: {}", lint_cmd.is_files_compatible());
    println!("    Resolved: {}", lint_cmd.get_resolved_command(&context));
    println!("    Used substitutions: {}", lint_cmd.get_used_substitutions(&context).join(", "));

    println!("  Test Command:");
    println!("    Run: {}", test_cmd.run);
    println!("    Uses file substitutions: {}", test_cmd.uses_file_substitutions());
    println!("    Is compatible: {}", test_cmd.is_files_compatible());
    println!("    Resolved: {}", test_cmd.get_resolved_command(&context));
    println!("    Used substitutions: {}", test_cmd.get_used_substitutions(&context).join(", "));

    println!("  Incompatible Command:");
    println!("    Run: {}", incompatible_cmd.run);
    println!("    Uses file substitutions: {}", incompatible_cmd.uses_file_substitutions());
    println!("    Is compatible: {}", incompatible_cmd.is_files_compatible());
    println!("    Used substitutions: {}", incompatible_cmd.get_used_substitutions(&context).join(", "));
    ...
    pre_commit_fmt.fail_text = Some("Code formatting check failed. Run 'cargo fmt' to fix.".to_string());

    let mut pre_commit_clippy = LefthookCommand::new("cargo clippy --all-targets --all-features -- {files}".to_string());
    pre_commit_clippy.files = Some("*.rs".to_string());
    pre_commit_clippy.priority = Some(5);
    pre_commit_clippy.fail_text = Some("Clippy found issues. Fix them before committing.".to_string());
    ...
    println!("    Parallel: {}", real_world_config.parallel.map(|p| if p { "Yes" } else { "No" }).unwrap_or_else(|| "Not set"));
    println!("    Commands by Priority:");
    for (name, cmd) in real_world_config.commands_by_priority() {
        println!("      {} (priority: {})", name, cmd.execution_priority());
        println!("        Run: {}", cmd.run);
        println!("        Resolved: {}", cmd.get_resolved_command(&context));
        println!("        Fail Text: {}", cmd.fail_text.as_deref().unwrap_or("None"));
    }
    ...
    let pre_commit_state = || GitState::new(GitRepoState::Normal, "feature/new-feature".to_string());
    ...
    println!("    Always skip (pre-commit): {}", skip_checker_pre_commit.check(Some(&always_skip), None));
    println!("    Never skip (pre-commit): {}", skip_checker_pre_commit.check(Some(&never_skip), None));

    // Test string conditions
    let pre_commit_skip = AdvancedSkipCondition::Conditions(vec![
        ConditionValue::String("pre-commit".to_string()),
    ]);
    let pre_push_skip = AdvancedSkipCondition::Conditions(vec![
        ConditionValue::String("pre-push".to_string()),
    ]);

    println!("    Skip on pre-commit (pre-commit state): {}", skip_checker_pre_commit.check(Some(&pre_commit_skip), None));
    println!("    Skip on pre-push (pre-commit state): {}", skip_checker_pre_commit.check(Some(&pre_push_skip), None));
    println!("    Skip on pre-push (pre-push state): {}", skip_checker_pre_push.check(Some(&pre_push_skip), None));

    // Test reference conditions
    let feature_branch_skip = AdvancedSkipCondition::Conditions(vec![
        ConditionValue::Reference {
            ref_pattern: "feature/*".to_string(),
        },
    ]);
    let main_branch_skip = AdvancedSkipCondition::Conditions(vec![
        ConditionValue::Reference {
            ref_pattern: "main".to_string(),
        },
    ]);

    println!("    Skip on feature/* (feature/new-feature): {}", skip_checker_pre_commit.check(Some(&feature_branch_skip), None));
    println!("    Skip on main (feature/new-feature): {}", skip_checker_pre_commit.check(Some(&main_branch_skip), None));
    println!("    Skip on main (main): {}", skip_checker_pre_push.check(Some(&main_branch_skip), None));

    // Test only conditions
    let only_pre_commit = AdvancedOnlyCondition::Conditions(vec![
        ConditionValue::String("pre-commit".to_string()),
    ]);
    let only_main_branch = AdvancedOnlyCondition::Conditions(vec![
        ConditionValue::Reference {
            ref_pattern: "main".to_string(),
        },
    ]);

    println!("    Only on pre-commit (pre-commit state): {}", skip_checker_pre_commit.check(None, Some(&only_pre_commit)));
    println!("    Only on pre-commit (pre-push state): {}", skip_checker_pre_push.check(None, Some(&only_pre_commit)));
    println!("    Only on main branch (feature/new-feature): {}", skip_checker_pre_commit.check(None, Some(&only_main_branch)));
    println!("    Only on main branch (main): {}", skip_checker_pre_push.check(None, Some(&only_main_branch)));
    ...
    println!("    Skip on pre-push (pre-commit state): {}", fmt_command.should_skip_advanced(&skip_checker_pre_commit));
    println!("    Skip on pre-push (pre-push state): {}", fmt_command.should_skip_advanced(&skip_checker_pre_push));

    println!("  Test Command:");
    println!("    Run: {}", test_command.run);
    println!("    Only on main (feature/new-feature): {}", test_command.should_skip_advanced(&skip_checker_pre_commit));
    println!("    Only on main (main): {}", test_command.should_skip_advanced(&skip_checker_pre_push));

    println!("  Lint Command:");
    println!("    Run: {}", lint_command.run);
    println!("    Skip if no staged changes (pre-commit state): {}", lint_command.should_skip_advanced(&skip_checker_pre_commit));
    ...
    for (name, cmd) in advanced_config.commands_by_priority() {
        ...
        let pre_commit_checker = SkipChecker::new(|| GitState::new(GitRepoState::Normal, "feature/new-feature".to_string()));
        let pre_push_checker = SkipChecker::new(|| GitState::new(GitRepoState::Normal, "main".to_string()));

        println!("        Skip on pre-commit (feature branch): {}", cmd.should_skip_advanced(&pre_commit_checker));
        println!("        Skip on pre-push (main branch): {}", cmd.should_skip_advanced(&pre_push_checker));
    }
    ...
    let mermaid_content = format!("# Git File States Diagram\n\n```mermaid\n{}\n```\n",
    ...
    let comprehensive_content = format!("# Git Comprehensive Diagram\n\n```mermaid\n{}\n```\n",
    ...
    let workflow_content = format!("# Git Commit Workflow\n\n```mermaid\n{}\n```\n",
        diagrams::export_commit_workflow_diagram());
    fs::write(&workflow_path, workflow_content)
        .expect("Failed to write commit workflow diagram");
    ...
    let skip_only_content = format!("# Git Skip/Only Conditions\n\n```mermaid\n{}\n```\n",
    ...
    println!("\n    📁 All diagrams exported to '{}' directory", diagrams_dir);
