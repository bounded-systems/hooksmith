use hooksmith::modules::functional_contract_pipeline::{
    high_performance_diff::{convenience, DiffStrategy, HighPerformanceDiffer},
...
/// Demonstrate different diffing strategies
fn demonstrate_diff_strategies(
...
fn demonstrate_performance_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (description, (observed, expected)) in test_cases {
        ...
        for (i, metrics) in sorted_results.iter().enumerate() {
            ...
            println!(
                "  {} {:?}: {}μs",
                rank, metrics.strategy, metrics.total_time_micros
            );
        }
        println!();
    }
...
fn create_simple_test_data() -> (
    Vec<hooksmith::modules::functional_contract_pipeline::types::ConcernSnapshot>,
    Vec<hooksmith::modules::functional_contract_pipeline::types::ExpectedSnapshot>,
) {
    use hooksmith::modules::functional_contract_pipeline::types::{
        ConcernSnapshot, ExpectedSnapshot,
    };
...
fn demonstrate_usage_patterns() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Strategy: {:?}, Time: {}μs",
        metrics.strategy, metrics.total_time_micros
    );
    println!();
    ...
    let (diff_set, metrics) =
        convenience::auto_diff(&create_array_test_data().0, &create_array_test_data().1);
    println!(
        "   Auto-selected: {:?}, Time: {}μs",
        metrics.strategy, metrics.total_time_micros
    );
    println!();
