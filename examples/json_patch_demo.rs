...
fn demonstrate_json_patch_generation(
...
fn demonstrate_json_patch_verification(
    pipeline: &FunctionalContractPipeline,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match pipeline.run_hook_with_json_patch(hook) {
        Ok(diff_set) => {
            if diff_set.is_valid() {
                ...
                println!(
                    "  ❌ Validation failed with {} differences",
                    diff_set.diff_count()
                );
                ...
                for diff in &diff_set.diffs {
                    ...
                    if let Some(patch_value) = diff.metadata.get("json_patch") {
                        if let Ok(patch) = serde_json::from_value::<Vec<json_patch::PatchOperation>>(
                            patch_value.clone(),
                        ) {
...
fn demonstrate_performance_comparison() {
    ...
    println!(
        ...
        patch.len() as f64 / json_patch_duration.as_secs_f64()
    );
}
