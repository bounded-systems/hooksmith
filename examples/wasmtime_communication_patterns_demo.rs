...
use crate::xtask::event_bus::{HooksmithEvent, EventBusConfig};
...
async fn await_direct_linking_demo(orchestrator: &mut HooksmithOrchestrator) -> Result<()> {
    ...
    match orchestrator.load_linked_components().await {
        Ok(_) => {
            ...
            println!("Linked components: {:?}", orchestrator.list_linked_components());
...
async fn await_event_driven_demo(orchestrator: &HooksmithOrchestrator) -> Result<()> {
    ...
    let file_content = orchestrator.read_file_via_events("examples/test_contract.json").await?;
    ...
    orchestrator.store_proof_via_events("examples/test_contract.json", &validation_result).await?;
...
async fn await_performance_comparison_demo(orchestrator: &HooksmithOrchestrator) -> Result<()> {
    ...
    println!("   Direct linking: {} iterations in {:?} (avg: {:?})", 
    ...
    for _ in 0..iterations {
        let _result = orchestrator.validate_contract_via_events(
            "performance_test",
            "test.json",
            contract_data,
            true,
...
async fn await_hybrid_workflow_demo(orchestrator: &HooksmithOrchestrator) -> Result<()> {
    ...
    for (file, content) in &file_contents {
        ...
        println!("   Validated {}: {:?} in {:?}", file, result.success, duration);
    }
    ...
    for (file, result, duration) in &validation_results {
        ...
        println!("   Stored proof for {} (validation took {:?})", file, duration);
    }
    ...
    let successful_validations = validation_results.iter()
        ...
        .count();
    let total_duration: Duration = validation_results.iter()
    ...
    println!("     Average validation time: {:?}", total_duration / total_files as u32);
...
pub async fn demo_shared_memory() -> Result<()> {
    ...
    println!("let component_a = Component::from_file(&engine, \"data-processor-a.component.wasm\")?;");
    ...
    println!("");
    println!("let component_b = Component::from_file(&engine, \"data-processor-b.component.wasm\")?;");
