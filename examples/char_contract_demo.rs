...
fn demo_character_classification() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &byte in &test_bytes {
        ...
        println!("  0x{:02X}: {} -> {}", byte, contract.description(), 
...
fn demo_file_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Line endings normalized: {}", result.line_endings_normalized);
...
fn demo_binary_heuristic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Text file: {} ({}% forbidden)", 
        if result.is_binary_heuristic() { "❌ Binary" } else { "✅ Text" },
    ...
    println!("  Binary-like file: {} ({}% forbidden)", 
        if result.is_binary_heuristic() { "❌ Binary" } else { "✅ Text" },
        result.forbidden_percentage);
