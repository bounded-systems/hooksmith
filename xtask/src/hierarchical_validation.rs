...
use hooksmith::modules::hierarchical_validation::{
    HierarchicalValidator, ValidationScope, ChangeScope, ValidationResult
};
...
/// Main entry point for the hierarchical validation CLI
...
async fn validate_changes(range: &str, repo: &PathBuf) -> Result<()> {
    ...
    // Detect changes
    let changes = validator.detect_changes(Some(range)).await
    ...
    let results = validator.validate_hierarchically(changes).await
    ...
    for result in &results {
        if result.validated {
            total_validated += 1;
            println!("✅ {:?} scope validated successfully ({}ms)", 
                result.scope, result.duration_ms);
        } else {
            total_failed += 1;
            println!("❌ {:?} scope validation failed ({}ms)", 
                result.scope, result.duration_ms);
...
async fn verify_validation_chain(commit: &str, repo: &PathBuf) -> Result<()> {
    ...
    let is_valid = validator.verify_validation_chain(commit).await
...
async fn show_validation_notes(commit: &str, repo: &PathBuf) -> Result<()> {
    ...
    let notes = validator.get_validation_notes(commit).await
...
async fn pre_commit_hook(repo: &PathBuf) -> Result<()> {
    ...
    if !output.status.success() {
        anyhow::bail!("Failed to get staged changes: {}", String::from_utf8_lossy(&output.stderr));
    }
    ...
    let changes = validator.detect_changes(Some(&format!("{}~1..{}", temp_commit, temp_commit))).await
        .context("Failed to detect changes")?;
...
async fn post_commit_hook(repo: &PathBuf) -> Result<()> {
    ...
    if !output.status.success() {
        anyhow::bail!("Failed to get current commit hash: {}", String::from_utf8_lossy(&output.stderr));
    }
    ...
    let changes = validator.detect_changes(Some(&format!("{}~1..{}", commit_hash, commit_hash))).await
        .context("Failed to detect changes")?;
    ...
    let failed_count = results.iter().filter(|r| !r.validated).count();
    if failed_count > 0 {
        println!("⚠️  Post-commit validation failed for {} scopes", failed_count);
        println!("   Validation notes have been stored in Git notes");
        println!("   Use 'xtask-contract-validate show {}' to view details", commit_hash);
    } else {
...
async fn create_temp_commit(repo: &PathBuf) -> Result<String> {
    ...
    if !output.status.success() {
        anyhow::bail!("Failed to create temp commit: {}", String::from_utf8_lossy(&output.stderr));
    }
    ...
    if !output.status.success() {
        anyhow::bail!("Failed to get temp commit hash: {}", String::from_utf8_lossy(&output.stderr));
    }
...
async fn cleanup_temp_commit(repo: &PathBuf, commit_hash: &str) -> Result<()> {
    ...
    if !output.status.success() {
        eprintln!("Warning: Failed to reset temp commit: {}", String::from_utf8_lossy(&output.stderr));
    }
