...
fn test_all_available_hooks() {
    ...
    for hook in lefthook::AVAILABLE_HOOKS {
        assert!(
            lefthook::is_known_hook(hook),
            "Hook '{}' should be known",
            hook
        );
    }
...
async fn test_all_hooks_schema_validation() {
    ...
    assert!(
        result.is_ok(),
        "Configuration with all hooks should pass schema validation"
    );
}
...
async fn test_comprehensive_config_generation() {
    ...
    assert!(
        result.is_ok(),
        "Should generate comprehensive configuration"
    );
...
fn test_hook_set_operations() {
    ...
    for hook in lefthook::AVAILABLE_HOOKS {
        assert!(
            hook_set.contains(hook),
            "Hook '{}' should be in the set",
            hook
        );
    }
