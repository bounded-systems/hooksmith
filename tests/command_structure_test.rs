...
fn test_exclude_patterns() {
    // Test string exclude
    let hook = lefthook::LefthookHook::new("echo test".to_string())
        .with_exclude(json!("*.tmp"));
    assert_eq!(hook.get_exclude_patterns(), vec!["*.tmp"]);

    // Test array of excludes
    let hook = lefthook::LefthookHook::new("echo test".to_string())
        .with_exclude(json!(["*.tmp", "*.log", "node_modules/"]));
    assert_eq!(hook.get_exclude_patterns(), vec!["*.tmp", "*.log", "node_modules/"]);
...
fn test_file_types() {
    ...
    assert_eq!(hook.file_types, Some(vec!["rust".to_string(), "toml".to_string()]));
}

#[test]
fn test_root_directory() {
    let hook = lefthook::LefthookHook::new("echo test".to_string())
        .with_root("src/".to_string());
...
fn test_priority_ordering() {
    let hook1 = lefthook::LefthookHook::new("echo first".to_string())
        .with_priority(1);
    
    let hook2 = lefthook::LefthookHook::new("echo second".to_string())
        .with_priority(10);
