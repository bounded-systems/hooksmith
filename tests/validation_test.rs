use hooksmith::modules::lefthook::{self, LefthookError, LefthookHook, LefthookResult};
...
#[test]
fn test_hook_configuration_validation() {
    // Test valid hook
...
fn test_configuration_validation() {
    ...
    // Test configuration with empty hook name
    let mut config = LefthookConfig::default();
    let mut hooks = HashMap::new();
...
#[test]
fn test_error_types() {
    // Test FilesIncompatible error
    let error = LefthookError::FilesIncompatible("test".to_string());
    assert_eq!(error.to_string(), "Files incompatible: test");

    // Test InvalidHookName error
    let error = LefthookError::InvalidHookName("invalid-hook".to_string());
    assert_eq!(error.to_string(), "Invalid hook name: invalid-hook");

    // Test MissingRequiredField error
    let error = LefthookError::MissingRequiredField("run".to_string());
    assert_eq!(error.to_string(), "Missing required field: run");

    // Test InvalidConfiguration error
    let error = LefthookError::InvalidConfiguration("test".to_string());
    assert_eq!(error.to_string(), "Invalid configuration: test");

    // Test SchemaValidation error
    let error = LefthookError::SchemaValidation("test".to_string());
    assert_eq!(error.to_string(), "Schema validation failed: test");
}
...
fn test_complex_validation_scenarios() {
    ...
    pre_commit_hooks.insert(
        "js-lint".to_string(),
        LefthookHook::new("npm run lint".to_string())
