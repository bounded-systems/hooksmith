...
impl WasmManager {
    ...
    pub async fn build_component(&self, config: WasmBuildConfig) -> Result<WasmBuildResult> {
        ...
        // Write WASM file
        let wasm_filename = config.wit_file
            .file_stem()
            .and_then(|s| s.to_str())
            .unwrap_or("component");
        ...
        let bindings_path = if config.generate_bindings {
            ...
            fs::write(&bindings_path, bindings).await
    ...
    pub async fn run_component(&mut self, config: WasmRunConfig) -> Result<WasmRunResult> {
        ...
        // Load WASM module
        let wasm_bytes = fs::read(&config.wasm_file).await
        ...
        // Get function
        let func = instance.get_func(&mut store, &config.function)
        ...
        let _results = func.call(&mut store, &[], &mut vec![])
    ...
    pub async fn generate_bindings(&self, config: WasmBuildConfig) -> Result<WasmBuildResult> {
        ...
        let bindings_filename = config.wit_file
            .file_stem()
            .and_then(|s| s.to_str())
            .unwrap_or("interface");
        ...
        // Write bindings file
        let bindings_path = config.output_dir.join(format!("{}_bindings.rs", bindings_filename));
        fs::write(&bindings_path, bindings).await
            .context("Failed to write bindings file")?;
    ...
    fn generate_placeholder_wasm(&self) -> Result<Vec<u8>> {
        ...
        let wasm_bytes = vec![
            ...
            // Export section
            0x07, 0x0a, 0x01, 0x04, 0x6d, 0x61, 0x69, 0x6e, 0x02, 0x00, // export "main" as function 0
            // Code section
