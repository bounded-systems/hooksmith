...
fn find_language_by_extension(extension: &str, languages: &HashMap<String, LanguageInfo>) -> Vec<String> {
...
fn find_language_by_filename(filename: &str, languages: &HashMap<String, LanguageInfo>) -> Vec<String> {
    let mut matches = Vec::new();
...
fn print_file_analysis(analyses: &[FileAnalysis]) {
    ...
    for analysis in high_confidence.iter().take(5) {
        println!("   {} (confidence: {:.1})", analysis.path.display(), analysis.confidence);
    }
...
fn main() -> ExitCode {
    ...
    let languages = match load_languages_yml() {
        Ok(languages) => {
            println!("✅ Loaded {} languages from GitHub Linguist", languages.len());
            languages
        }
        Err(e) => {
    ...
    match args[1].as_str() {
        ...
        "analyze" | "suggestions" | "full" => {
            ...
            let analyses: Vec<FileAnalysis> = files.iter()
