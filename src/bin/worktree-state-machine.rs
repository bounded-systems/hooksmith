...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    log_header("worktree-state-machine");
    println!();
    
    // TODO: Implement functionality from ./scripts/worktree-state-machine.sh
    log_info("Converting from shell script: ./scripts/worktree-state-machine.sh");
    
    // Add specific implementation based on script type
    match "[0;34m[INFO][0m Analyzing: worktree-state-machine
worktree_management" {
        "worktree_management" => {
