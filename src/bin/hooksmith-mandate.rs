...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(line) = lines.next() {
        ...
        // Check cache first
        let cache_key = compute_cache_key(&cli.version, &cli.contract_ref, &object_selector, &object.logical_path);
...
fn load_contract(repo: &Repository, contract_ref: &str) -> Result<Contract, Box<dyn std::error::Error>> {
