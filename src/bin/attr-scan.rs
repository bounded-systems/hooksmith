    ...
    for classification in classifications {
        ...
        for filter in &filters {
            ...
            let matches = match attr {
                "linguist-language" => {
                    classification.linguist_language.as_ref().map_or(false, |lang| lang == value)
                }
...
fn print_matrix_command(classifications: &[FileClassification], args: &[String]) {
    ...
    for classification in classifications {
        let group_key = match group_by.as_str() {
            "linguist-language" => {
                classification.linguist_language.clone().unwrap_or_else(|| "unknown".to_string())
            }
            "linguist-vendored" => {
                if classification.is_vendored { "vendored".to_string() } else { "not-vendored".to_string() }
            }
            "linguist-generated" => {
                if classification.is_generated { "generated".to_string() } else { "not-generated".to_string() }
            }
            "linguist-documentation" => {
                if classification.is_documentation { "documentation".to_string() } else { "not-documentation".to_string() }
            }
            "linguist-detectable" => {
                if classification.is_detectable { "detectable".to_string() } else { "not-detectable".to_string() }
            }
            _ => "unknown".to_string(),
        };
        
        groups.entry(group_key).or_insert_with(Vec::new).push(classification);
    }
...
fn print_stats_command(classifications: &[FileClassification]) {
    ...
    println!("  Vendored files: {} ({:.1}%)", vendored_count, (vendored_count as f64 / classifications.len() as f64) * 100.0);
    println!("  Generated files: {} ({:.1}%)", generated_count, (generated_count as f64 / classifications.len() as f64) * 100.0);
    println!("  Documentation: {} ({:.1}%)", documentation_count, (documentation_count as f64 / classifications.len() as f64) * 100.0);
    println!("  Detectable files: {} ({:.1}%)", detectable_count, (detectable_count as f64 / classifications.len() as f64) * 100.0);
    println!("  Unclassified: {} ({:.1}%)", unclassified_count, (unclassified_count as f64 / classifications.len() as f64) * 100.0);
    println!();
    ...
    if vendored_count > 0 {
        println!("  • Exclude {} vendored files from linting/formatting", vendored_count);
    }
    
    if generated_count > 0 {
        println!("  • Skip diff review for {} generated files", generated_count);
    }
    
    if documentation_count > 0 {
        println!("  • Route {} documentation files to doc validation tools", documentation_count);
    }
    ...
    if *json_files > 0 {
        println!("  • Validate {} JSON files with schema validation", json_files);
    }
}
...
fn print_workflow_examples(classifications: &[FileClassification]) {
    ...
    // Rust files
    let rust_files: Vec<_> = classifications.iter()
        .filter(|c| c.linguist_language.as_ref().map_or(false, |lang| lang == "Rust"))
        .collect();
    
    if !rust_files.is_empty() {
        println!("🔧 Rust Workflow:");
        println!("  # Format Rust files");
        println!("  cargo fmt --check -- {}", rust_files.iter().map(|c| c.path.display().to_string()).collect::<Vec<_>>().join(" "));
        println!("  # Clippy check");
        println!("  cargo clippy -- {}", rust_files.iter().map(|c| c.path.display().to_string()).collect::<Vec<_>>().join(" "));
        println!();
    }
    
    // Non-vendored files
    let non_vendored: Vec<_> = classifications.iter()
        .filter(|c| !c.is_vendored)
        .collect();
    
    if !non_vendored.is_empty() {
        println!("📝 Non-Vendored Files:");
        println!("  # Apply linting to non-vendored files");
        println!("  echo '{}' | xargs my-linter", non_vendored.iter().map(|c| c.path.display().to_string()).collect::<Vec<_>>().join(" "));
        println!();
    }
    
    // Generated files
    let generated_files: Vec<_> = classifications.iter()
    ...
    if !generated_files.is_empty() {
        ...
        println!("  git diff -- {}", generated_files.iter().map(|c| c.path.display().to_string()).collect::<Vec<_>>().join(" "));
        println!("  # Validate generated files match source");
        println!("  make generate && git diff --exit-code -- {}", generated_files.iter().map(|c| c.path.display().to_string()).collect::<Vec<_>>().join(" "));
        println!();
    }
    
    // Documentation files
    let doc_files: Vec<_> = classifications.iter()
    ...
    if !doc_files.is_empty() {
        ...
        println!("  echo '{}' | xargs markdownlint", doc_files.iter().map(|c| c.path.display().to_string()).collect::<Vec<_>>().join(" "));
        println!();
    }
}
...
fn main() -> ExitCode {
    ...
    if args.len() < 2 {
        ...
        println!();
        println!("Usage:");
        println!("  {} files [options]     - List files with filters", args[0]);
        println!("  {} matrix [options]    - Show classification matrix", args[0]);
    ...
    let classifications: Vec<FileClassification> = if files.len() > 1000 {
        ...
        let files_chunks: Vec<Vec<PathBuf>> = files.chunks(files.len() / num_cpus::get().max(1))
