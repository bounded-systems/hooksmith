use hooksmith::{
    get_worktrees, print_status, run_git_command, run_git_command_in_dir, CleanupDecision,
};
use std::path::Path;
...
fn analyze_worktree(worktree_path: &str) -> Result<CleanupDecision, String> {
    ...
    let commit_count =
        run_git_command_in_dir(&["log", "--oneline", "--since=1 week ago"], worktree_path)
    ...
    // Check for conflicts
    let conflicts =
        run_git_command_in_dir(&["diff", "--name-only", "--diff-filter=U"], worktree_path)
            .unwrap_or_else(|_| "".to_string());
    let rebase_status =
        run_git_command_in_dir(&["status"], worktree_path).unwrap_or_else(|_| "".to_string());
    ...
    if behind_count > 5 {
        print_status(
            "WARNING",
            &format!("Branch is {} commits behind main", behind_count),
        );
    ...
    let remote_check =
        run_git_command_in_dir(&["ls-remote", "--heads", "origin", &branch], worktree_path);
    if remote_check.is_ok() && !remote_check.unwrap().is_empty() {
        ...
        if merged_branches
            .lines()
            .any(|line| line.trim() == format!("* {}", branch))
        {
