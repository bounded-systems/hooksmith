...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if current_branch == "main" {
        ...
        if ahead_commits > 0 {
            log_warning(&format!("⚠️  Pre-commit: Main is ahead of origin/main by {} commit(s)", ahead_commits));
            log_info("🔄 Running main cleanup workflow...");

            // Run the cleanup script
            let cleanup_result = Command::new("./scripts/ensure-clean-main.sh")
                .status();
            ...
            match cleanup_result {
                ...
                Ok(status) => {
                    log_error(&format!("❌ Pre-commit: Main cleanup failed with exit code: {}", status));
                    std::process::exit(1);
                }
                Err(e) => {
                    log_error(&format!("❌ Pre-commit: Failed to run cleanup script: {}", e));
                    std::process::exit(1);
                }
            }
        } else {
        ...
        log_info(&format!("Skipping pre-commit check - not on main branch (current: {})", current_branch));
    }
