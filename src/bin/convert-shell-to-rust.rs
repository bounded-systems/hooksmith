...
fn create_rust_binary(script_path: &Path, script_type: &str) -> Result<(), Box<dyn std::error::Error>> {
    let script_name = script_path.file_stem()
        .and_then(|s| s.to_str())
        .unwrap_or("unknown");
...
fn extract_functions(script_path: &Path) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    ...
    for line in content.lines() {
        if let Some(function_name) = line.trim().strip_suffix("()") {
            if function_name.chars().next().map_or(false, |c| c.is_ascii_alphabetic() || c == '_') {
...
fn create_conversion_summary(
    ...
    summary_content.push_str(&format!("- **Rust Binary**: `src/bin/{}.rs`\n", script_name));
    ...
    log_info_colored(&format!("Added to conversion summary: {}", summary_file.display()));
    Ok(())
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    log_info_colored(&format!("Found {} shell scripts to convert", shell_scripts.len()));
    println!();
    ...
    log_success_colored(&format!("Converted {} shell scripts to Rust", converted_count));
    println!();
