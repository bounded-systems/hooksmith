...
use git2::Repository;
use hooksmith_core::{
    git_pipeline::{GitPipeline, ConcernSelector, ToolFingerprint},
    object_names_validator::{ObjectNamesValidator, load_contract},
};
        ...
        Commands::Select { ref_name, selector } => {
            ...
            for object in objects {
                println!("  {} ({:?}) - {}", 
                    object.oid, 
                    object.kind,
                    object.logical_path.as_ref().map(|p| p.to_string_lossy()).unwrap_or_default()
                );
            }
        }
