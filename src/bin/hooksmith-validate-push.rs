...
fn main() -> Result<()> {
    ...
    if !refs_output.status.success() {
        bail!("Failed to list Git references: {}", String::from_utf8_lossy(&refs_output.stderr));
    }
    ...
    if !remotes_output.status.success() {
        bail!("Failed to list Git remotes: {}", String::from_utf8_lossy(&remotes_output.stderr));
    }
    ...
    if !worktrees_output.status.success() {
        bail!("Failed to list Git worktrees: {}", String::from_utf8_lossy(&worktrees_output.stderr));
    }
    ...
    if worktree_count > 1 {
        warnings.push(format!("Multiple worktrees ({}) - ensure all are clean", worktree_count));
    }
